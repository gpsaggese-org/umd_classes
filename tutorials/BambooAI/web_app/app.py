...
import tutorials.BambooAI.web_app.app as tbweapap
"""
...
def cleanup_threads(debug_mode=False):
    ...
    if os.path.exists(favorites_dir):
        favorite_thread_ids = {
            d
            for d in os.listdir(favorites_dir)
...
def clear_datasets_folder():
    ...
    if os.path.exists(favorites_base_dir):
        try:
            favorite_thread_ids = {
                d
                for d in os.listdir(favorites_base_dir)
                if os.path.isdir(os.path.join(favorites_base_dir, d))
            }
            app.logger.info(
                f"Favorite thread IDs for dataset cleanup: {list(favorite_thread_ids)}"
            )
        except Exception as e:
            app.logger.error(
                f"Error reading favorites directory {favorites_base_dir}: {e}"
            )

    if not os.path.exists(datasets_dir):
        app.logger.info(
            f"'{datasets_dir}' folder does not exist, no need to clear."
        )
        return
        ...
        for item_name in os.listdir(datasets_dir):
            ...
            try:
                if os.path.isfile(item_path) or os.path.islink(item_path):
                    ...
                    if item_name == "generated" and os.path.exists(
                        generated_datasets_base_dir
                    ):
                        app.logger.info(
                            f"Processing 'generated' subdirectory: {generated_datasets_base_dir}"
                        )
                        for generated_thread_id_dir_name in os.listdir(
                            generated_datasets_base_dir
                        ):
                            thread_specific_dir_path = os.path.join(
                                generated_datasets_base_dir,
                                generated_thread_id_dir_name,
                            )
                            if os.path.isdir(thread_specific_dir_path):
                                # This directory name is treated as the thread_id
                                if (
                                    generated_thread_id_dir_name
                                    not in favorite_thread_ids
                                ):
                                    shutil.rmtree(thread_specific_dir_path)
                                    app.logger.info(
                                        f"Deleted non-favorited generated dataset directory: {thread_specific_dir_path}"
                                    )
                                else:
                                    app.logger.info(
                                        f"Kept favorited generated dataset directory: {thread_specific_dir_path}"
                                    )
                    elif item_name != "generated":
                        app.logger.warning(
                            f"Skipping deletion of non-'generated' subdirectory: {item_path}. Adjust logic if this should be deleted."
                        )
            except Exception as e:
                app.logger.error(
                    f"Failed to process item {item_path}. Reason: {e}"
                )
        app.logger.info(
            f"Selective cleanup of '{datasets_dir}' folder completed."
        )
    except Exception as e:
...
try:
    ...
    if os.path.exists(bamboo_ai_path):
        ...
        raise ImportError(
            "Could not find bambooai package. Please either install via pip or ensure you're running from the correct directory in the cloned repository."
        )
...
EXECUTOR_API_UPLOAD_AUX_URL = (
    f"{EXECUTOR_API_BASE_URL}/file_utils/upload_aux_dataset"
)
EXECUTOR_API_REMOVE_AUX_URL = (
    f"{EXECUTOR_API_BASE_URL}/file_utils/remove_aux_dataset"
)
EXECUTOR_API_DOWNLOAD_GENERATED_URL = (
    f"{EXECUTOR_API_BASE_URL}/download_generated_dataset"
)
# Get execution mode from environment variable or default to 'local'
GLOBAL_EXECUTION_MODE = os.getenv(
    "EXECUTION_MODE", "local"
)  # Default to 'local' if not set

# Create an instance of the ExecutorAPIClient
executor_client = executor_client.ExecutorAPIClient(
    base_url=EXECUTOR_API_BASE_URL
)
...
def get_bamboo_ai(session_id, df=None):
    ...
    ontology_path = prefs.get(
        "ontology_path", DF_ONTOLOGY
    )  # Default to constant if not set
...
def load_csv_with_datetime(file_path):
    ...
    def parse_and_remove_tz(series):
        return pd.to_datetime(
            series, format="%Y-%m-%d %H:%M:%S%z", utc=True
        ).dt.tz_localize(None)
...
def load_dataframe_to_bamboo_ai_instance(
    session_id, df=None, file=None, execution_mode="local"
):
    ...
    if execution_mode == "api":
        ...
        try:
            ...
            executor_response = requests.post(
                EXECUTOR_API_UPLOAD_URL, files=files, data={"df_id": new_df_id}
            )
...
def start_new_conversation(session_id):
    ...
    if old_ontology_path and os.path.exists(old_ontology_path):
        try:
            ...
            app.logger.info(
                f"Removed old ontology file during new conversation: {old_ontology_path}"
            )
        except Exception as e:
            app.logger.error(
                f"Failed to remove old ontology file {old_ontology_path} during new conversation: {str(e)}"
            )
    ...
    app.logger.info(
        f"BambooAI instance reset for session {session_id}, auxiliary datasets list cleared, ontology cleared, and Datasets folder content cleared."
    )
...
def transform_sweatstack_longitudinal_data(df):
    ...
    df["datetime"] = pd.to_datetime(df.index).tz_localize(
        None
    )  # Remove timezone info to convert to local time
    ...
    if "athlete_id" in df.columns:
        ...
        unique_activities = (
            df.groupby(["athlete_id", "activity_id"])["datetime"]
            .min()
            .sort_values()
        )
        activity_mapping = {}
        ...
        df["activity_id"] = df.apply(
            lambda row: activity_mapping.get(
                (row["athlete_id"], row["activity_id"]), row["activity_id"]
            ),
            axis=1,
        )
        ...
        athlete_mapping = {
            old_id: new_id for new_id, old_id in enumerate(unique_athletes, 1)
        }
        df["athlete_id"] = df["athlete_id"].map(athlete_mapping)
    else:
        # Single user scenario
        unique_activities = (
            df.groupby("activity_id")["datetime"].min().sort_values()
        )
        activity_mapping = {
            old_id: new_id
            for new_id, old_id in enumerate(unique_activities.index, 1)
        }
    ...
    if "duration" in df.columns and "speed" in df.columns:
        df["distance_increment"] = (
            df["duration"].dt.total_seconds() * df["speed"]
        )

        # Calculate cumulative distance per activity (and per athlete if multi-user)
        if "athlete_id" in df.columns:
            df["distance"] = df.groupby(["athlete_id", "activity_id"])[
                "distance_increment"
            ].cumsum()
        else:
            df["distance"] = df.groupby("activity_id")[
                "distance_increment"
            ].cumsum()
...
def index():
    common_context = {
        "sweatstack_enabled": bool(
            SWEATSTACK_CLIENT_ID and SWEATSTACK_CLIENT_SECRET
        ),
        ...
        "sweatstack_default_metrics": [
            ss.Metric.power,
            ss.Metric.speed,
            ss.Metric.heart_rate,
        ],
    }
    ...
    if session_id in bamboo_ai_instances:
        ...
        try:
            bamboo_ai_instances[session_id] = BambooAI(
                ...
                if hasattr(current_instance, "df")
                else None,
                user_id=USER_ID,
                ...
                if hasattr(current_instance, "df_id")
                else None,
                auxiliary_datasets=current_instance.auxiliary_datasets
                if hasattr(current_instance, "auxiliary_datasets")
                else [],
            )
            print(
                f"[DEBUG] Successfully updated BambooAI instance with planning={planning_enabled}"
            )
        except Exception as e:
...
def update_ontology():
    ...
    prefs = user_preferences.get(
        session_id, {"planning": False, "ontology_path": None}
    )
    old_ontology_path = prefs.get("ontology_path")
    ...
    if "ontology_file" in request.files:
        ...
        return jsonify(
            {
                "message": "Invalid request, must include ontology_file or clear ontology_path"
            }
        ), 400
    ...
    if old_ontology_path and os.path.exists(old_ontology_path):
        try:
            ...
            print(
                f"[ERROR] Failed to remove old ontology file {old_ontology_path}: {str(e)}"
            )
    ...
    if session_id in bamboo_ai_instances:
        ...
        try:
            ...
            print(
                f"[DEBUG] Successfully updated BambooAI instance with ontology_path={ontology_path}"
            )
        except Exception as e:
            print(f"[ERROR] Failed to update BambooAI instance: {str(e)}")
            return jsonify({"error": "Failed to update BambooAI instance"}), 500
...
def get_ontology_state():
    ...
    return jsonify(
        {"ontology_enabled": bool(ontology_path), "ontology_path": ontology_path}
    )
...
def upload_file():
    ...
    if file and (
        ...
        filepath = os.path.join(
            user_path("temp"), f"{session_id}_{file.filename}"
        )
        file.save(filepath)
...
def remove_primary_dataset():
    ...
    if not bamboo_ai_instance or bamboo_ai_instance.df_id is None:
        return jsonify(
            {"message": "No primary dataset is currently loaded."}
        ), 400
    ...
    try:
        ...
        ontology_path_pref = (
            prefs.get("ontology_path", DF_ONTOLOGY) if prefs else DF_ONTOLOGY
        )
        ...
        app.logger.info(
            f"Primary dataset removed and BambooAI instance reset for session {session_id}."
        )
        return jsonify({"message": "Primary dataset removed successfully."}), 200

    except Exception as e:
        app.logger.error(
            f"Error removing primary dataset for session {session_id}: {str(e)}"
        )
        return jsonify(
            {"message": f"Error removing primary dataset: {str(e)}"}
        ), 500
...
def upload_auxiliary_dataset():
    ...
    if "file" not in request.files:
        return jsonify(
            {"message": "No file part in request for auxiliary dataset."}
        ), 400
    file_to_upload = request.files[
        "file"
    ]  # Renamed to avoid conflict if passed to another function
    if file_to_upload.filename == "":
        return jsonify(
            {"message": "No file selected for auxiliary dataset."}
        ), 400
    ...
    if file_to_upload and (
        file_to_upload.filename.endswith(".csv")
        or file_to_upload.filename.endswith(".parquet")
    ):
        ...
        try:
            if GLOBAL_EXECUTION_MODE == "api":
                if not EXECUTOR_API_UPLOAD_AUX_URL:
                    return jsonify(
                        {
                            "message": "Executor API URL for aux upload not configured."
                        }
                    ), 500
                ...
                response = requests.post(
                    EXECUTOR_API_UPLOAD_AUX_URL, files=files_for_executor
                )
                ...
                if "filepath" not in api_response_data:
                    return jsonify(
                        {
                            "message": "Executor API did not return filepath for auxiliary dataset."
                        }
                    ), 500
                ...
                local_filepath = os.path.join(
                    datasets_dir, file_to_upload.filename
                )
            ...
            user_preferences[session_id]["auxiliary_datasets"] = (
                aux_datasets_list
            )
            ...
            if session_id in bamboo_ai_instances:
                ...
                app.logger.info(
                    f"BambooAI instance for session {session_id} updated with new auxiliary dataset list."
                )
            ...
            app.logger.error(
                f"Error communicating with executor API for aux upload: {str(e)}",
                exc_info=True,
            )
            return jsonify(
                {"message": f"API communication error: {str(e)}"}
            ), 500
        except Exception as e:
            app.logger.error(
                f"Error processing auxiliary dataset upload: {str(e)}",
                exc_info=True,
            )
            return jsonify(
                {"message": f"Error processing auxiliary dataset: {str(e)}"}
            ), 500
    else:
        return jsonify(
            {
                "message": "Invalid file type for auxiliary dataset. Must be .csv or .parquet."
            }
        ), 400
...
def remove_auxiliary_dataset():
    ...
    if not file_path_to_remove:
        return jsonify(
            {"message": "File path is required to remove auxiliary dataset."}
        ), 400

    prefs = user_preferences.get(session_id)
    if not (
        prefs
        and "auxiliary_datasets" in prefs
        and file_path_to_remove in prefs["auxiliary_datasets"]
    ):
    ...
    try:
        if GLOBAL_EXECUTION_MODE == "api":
            if not EXECUTOR_API_REMOVE_AUX_URL:
                return jsonify(
                    {
                        "message": "Executor API URL for aux removal not configured."
                    }
                ), 500

            response = requests.post(
                EXECUTOR_API_REMOVE_AUX_URL,
                json={"file_path": file_path_to_remove},
            )
            response.raise_for_status()  # Will raise an exception for 4xx/5xx errors
            # Executor API handles actual file deletion. We trust its response.
            app.logger.info(
                f"Request to remove auxiliary dataset '{file_path_to_remove}' sent to executor."
            )
            ...
            if not requested_file_abs.startswith(datasets_dir_abs):
                app.logger.warning(
                    f"Attempt to remove file outside local datasets directory: {file_path_to_remove}"
                )
                return jsonify(
                    {"message": "Invalid file path for local removal."}
                ), 403

            if os.path.exists(file_path_to_remove):
                os.remove(file_path_to_remove)
                app.logger.info(
                    f"Locally removed auxiliary dataset file: {file_path_to_remove}"
                )
            else:
                app.logger.warning(
                    f"Local auxiliary dataset file not found for removal, but removing from list: {file_path_to_remove}"
                )
        ...
        if session_id in bamboo_ai_instances:
            ...
            app.logger.info(
                f"BambooAI instance for session {session_id} updated after removing auxiliary dataset."
            )

        return jsonify(
            {
                "message": f'Auxiliary dataset "{os.path.basename(file_path_to_remove)}" processed for removal.',
                "remaining_aux_count": len(prefs["auxiliary_datasets"]),
            }
        ), 200

    except requests.RequestException as e:
        app.logger.error(
            f"Error communicating with executor API for aux removal: {str(e)}",
            exc_info=True,
        )
        return jsonify(
            {"message": f"API communication error during removal: {str(e)}"}
        ), 500
    except Exception as e:
        app.logger.error(
            f"Error removing auxiliary dataset '{file_path_to_remove}': {str(e)}",
            exc_info=True,
        )
...
def get_primary_dataset_preview():
    ...
    if not bamboo_ai_instance or bamboo_ai_instance.df_id is None:
        app.logger.info(
            f"Primary dataset preview requested for session {session_id}, but no DataFrame found."
        )
        error_df = pd.DataFrame(
            [{"Info": "No primary dataset is currently loaded or available."}]
        )
    ...
    try:
        ...
        df_id = (
            bamboo_ai_instance.df_id
            if hasattr(bamboo_ai_instance, "df_id")
            else None
        )
        ...
        df_html = df_sample_for_preview.to_html(
        ...
        app.logger.error(
            f"Error generating primary dataset preview for session {session_id}: {str(e)}"
        )
        error_df = pd.DataFrame(
            [
                {
                    "Error": f"Could not generate preview for the primary dataset: {str(e)}"
                }
            ]
        )
        df_html = error_df.to_html(classes="dataframe", border=0, index=False)
...
def get_dataset_preview():
    ...
    if not file_path_to_preview:
        return jsonify(
            {"message": "File path is required for auxiliary dataset preview."}
        ), 400
    ...
    if not (
        prefs
        and "auxiliary_datasets" in prefs
        and file_path_to_preview in prefs["auxiliary_datasets"]
    ):
        app.logger.warning(
        ...
        error_df = pd.DataFrame(
            [
                {
                    "Error": f"File not authorized or not found for preview: {os.path.basename(file_path_to_preview)}"
                }
            ]
        )
        df_html = error_df.to_html(classes="dataframe", border=0, index=False)
    ...
    try:
        ...
        error_df = pd.DataFrame(
            [
                {
                    "Error": f"Error generating preview for {os.path.basename(file_path_to_preview)}."
                }
            ]
        )
        df_html = error_df.to_html(classes="dataframe", border=0, index=False)
...
def query():
    ...
    if user_code:
        ...
        user_input = bamboo_ai_instance.prompts.ideas_explorer.format(
    ...
    def generate():
        while (
            thread.is_alive()
            or not bamboo_ai_instance.output_manager.output_queue.empty()
        ):
            try:
...
def submit_rank():
    ...
    if bamboo_ai_instance.vector_db:
        ...
        bamboo_ai_instance.output_manager.output_queue.put(
            json.dumps({"system_message": "Vector database is not enabled"})
        )
        ...
        return jsonify(
            {
                ...
                "task_included": bool(
                    task
                ),  # Let client know if task was included
            }
        ), 200
...
def get_threads():
    ...
    try:
        ...
        if not os.path.exists(favourites_dir):
            app.logger.warning(
                f"Favorites directory does not exist: {favourites_dir}"
            )
        ...
        thread_dirs = [
            d
            for d in os.listdir(favourites_dir)
        ...
        for thread_id in thread_dirs:
            ...
            for chain_file in chain_files:
                try:
                    ...
                    if not newest_timestamp or (
                        timestamp and timestamp > newest_timestamp
                    ):
                    ...
                    app.logger.error(
                        f"Error processing chain file {chain_file}: {str(e)}"
                    )
                    continue
        ...
        threads_info.sort(
            key=lambda t: t.get("newest_timestamp", ""), reverse=True
        )
...
def load_thread(thread_id, chain_id):
    ...
    try:
            ...
            try:
                ...
                response = {
                    ...
                    "chain_id": chain_data.get(
                        "chain_id", chain_id
                    ),  # Default to URL parameter if missing
        ...
        if not responses:
            ...
            return jsonify(
                {"error": "Failed to load any responses from the thread"}
            ), 500
...
def get_chain_preview(thread_id, chain_id):
    ...
    try:
        ...
        chain_file = user_path(
            "storage", "favourites", thread_id, f"{chain_id}.json"
        )
        ...
        if plotly_data:
            ...
            return jsonify(
                {"threadId": thread_id, "chainId": chain_id, "hasPlotly": False}
            ), 200

    except Exception as e:
        app.logger.error(
            f"Error getting chain preview for {thread_id}/{chain_id}: {str(e)}"
        )
        return jsonify({"error": f"Server error: {str(e)}"}), 500
...
def submit_feedback():
    ...
    try:
        if os.path.exists(feedback_file):
            ...
            app.logger.debug(
                f"Loaded existing feedback from {feedback_file}: {len(feedback_list)} entries"
            )
    except (json.JSONDecodeError, IOError) as e:
        app.logger.warning(
            f"Failed to read {feedback_file}: {str(e)}. Initializing empty list."
        )
...
def download_generated_dataset():
    ...
    app.logger.info(
        f"Attempting to download generated dataset: {file_path_param} in mode: {GLOBAL_EXECUTION_MODE}"
    )

    if GLOBAL_EXECUTION_MODE == "api":
        if not EXECUTOR_API_DOWNLOAD_GENERATED_URL:
            app.logger.error(
                "Executor API URL for generated dataset download not configured."
            )
            return jsonify(
                {"error": "Executor API URL for download not configured."}
            ), 500
        try:
            ...
            app.logger.info(
                f"Fetching from executor API: {executor_download_url}"
            )
            ...
            return jsonify(
                {"error": f"Failed to fetch file from remote service: {str(e)}"}
            ), 502  # Bad Gateway
        except Exception as e:
            app.logger.error(
                f"Unexpected error during API mode download: {str(e)}"
            )
            return jsonify(
        ...
        requested_file_abs = os.path.abspath(
            os.path.join(base_download_dir, file_path_param)
        )

        allowed_prefix = os.path.abspath(
            os.path.join(base_download_dir, user_path("datasets", "generated"))
        )

        if not requested_file_abs.startswith(allowed_prefix):
            app.logger.warning(
                f"Access denied for local download: {file_path_param}. Resolved path {requested_file_abs} is outside allowed prefix {allowed_prefix}."
            )
            return jsonify({"error": "Access denied or invalid file path."}), 403

        if not os.path.exists(requested_file_abs) or not os.path.isfile(
            requested_file_abs
        ):
            app.logger.error(
                f"Local file not found for download: {requested_file_abs}"
            )
        ...
        try:
            ...
            app.logger.info(
                f"Serving local file: directory='{directory}', filename='{filename}'"
            )
            return send_from_directory(directory, filename, as_attachment=True)
        except Exception as e:
            app.logger.error(
                f"Error serving local file {file_path_param}: {str(e)}"
            )
...
def search_threads():
    ...
    if not session_id or session_id not in bamboo_ai_instances:
        return jsonify(
            {"error": "Session not found or BambooAI not initialized"}
        ), 400
    ...
    try:
        if (
            not hasattr(bamboo_ai_instance, "vector_db_wrapper")
            or not bamboo_ai_instance.vector_db
        ):
            return jsonify(
                {
                    "search_results": [],
                    "message": "Vector DB not configured for this session.",
                }
            ), 200

        # Call the new function and adjust the key in the returned JSON
        search_results = bamboo_ai_instance.vector_db_wrapper.search_for_results(
...
def sweatstack_authorize():
    return redirect(
        f"https://app.sweatstack.no/oauth/authorize?client_id={SWEATSTACK_CLIENT_ID}&scope=data:read,profile&redirect_uri={request.url_root}sweatstack/oauth-callback&prompt=none"
    )
...
def sweatstack_callback():
    ...
    try:
        ...
        return jsonify(
            {"error": "Failed to exchange SweatStack code for token"}
        ), 500
    ...
    try:
        ...
        for user_id in selected_users:
            try:
                ...
                app.logger.warning(
                    f"Error loading data for user {user_id}: {str(e)}"
                )
                # Continue with other users

        if not all_dfs:
            return jsonify(
                {"error": "No data could be loaded for any selected users"}
            ), 400
        ...
        return jsonify(
            {"error": f"Failed to load SweatStack data: {str(e)}"}
        ), 500
...
def sweatstack_remove_data():
    ...
    try:
        ...
        if not bamboo_ai_instance or bamboo_ai_instance.df_id is None:
            return jsonify(
                {"message": "No SweatStack data is currently loaded."}
            ), 400
        ...
        app.logger.info(
            f"SweatStack data removed and BambooAI instance reset for session {session_id}."
        )
        return jsonify({"message": "SweatStack data removed successfully."}), 200

    except Exception as e:
        app.logger.error(
            f"Error removing SweatStack data for session {session_id}: {str(e)}"
        )
        return jsonify(
            {"error": f"Error removing SweatStack data: {str(e)}"}
        ), 500
