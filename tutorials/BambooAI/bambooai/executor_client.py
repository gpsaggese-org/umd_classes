...
import tutorials.BambooAI.bambooai.executor_client as tbbaexcl
"""
...
class ExecutorAPIClient:
    ...
    def compute_dataframe_sample(
        self, df_id: str, order_by: str = "Datetime", ascending: bool = False
    ...
    def dataframe_summary_to_string(self, df_id: str) -> Optional[str]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/df_utils/df_summary", json={"df_id": df_id}
            )
            response.raise_for_status()
            result = response.json()
    ...
    def dataframe_to_string(
        ...
        try:
            response = requests.post(
                ...
                json={"df_id": df_id, "num_rows": num_rows},
            )
            response.raise_for_status()
            result = response.json()
            ...
            if "error" in result:
                self.log_to_file(
                    f"Error converting to string: {result['error']}"
                )
                return None

            self.log_to_file(
                f"Successfully converted to string for df_id={df_id}"
            )
            return result["data"]
    ...
    def get_dataframe_columns(self, df_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/df_utils/df_columns", json={"df_id": df_id}
            )
            response.raise_for_status()
            result = response.json()
    ...
    def aux_datasets_to_string(
        self, file_paths: List[str], num_rows: int = 5
    ) -> Optional[str]:
        """Call the executor API to get string representation of auxiliary datasets."""
        self.log_to_file(
            f"Attempting to get aux datasets string for paths: {file_paths}, num_rows: {num_rows}"
        )
        try:
            response = requests.post(
            ...
            if "error" in result:
                self.log_to_file(
                    f"Error getting aux datasets string: {result['error']}"
                )
                return None  # Or consider returning result['error'] to propagate the message

            self.log_to_file(
                f"Successfully got aux datasets string for paths: {file_paths}"
            )
            return result.get("data")  # Expects {'data': 'output_string'}

        except requests.RequestException as e:
            self.log_to_file(
                f"Failed to get aux datasets string via API: {str(e)}"
            )
            return None
    ...
    def get_aux_datasets_columns(self, file_paths: List[str]) -> Optional[str]:
        ...
        self.log_to_file(
            f"Attempting to get aux datasets columns for paths: {file_paths}"
        )
        try:
            response = requests.post(
            ...
            if "error" in result:
                self.log_to_file(
                    f"Error getting aux datasets columns: {result['error']}"
                )
                return None

            self.log_to_file(
                f"Successfully got aux datasets columns for paths: {file_paths}"
            )
            return result.get("data")  # Expects {'data': 'columns_string'}

        except requests.RequestException as e:
            self.log_to_file(
                f"Failed to get aux datasets columns via API: {str(e)}"
            )
            return None

    def compute_aux_dataset_sample(
        self, file_paths: List[str], num_rows: int = 100
    ) -> Optional[List[str]]:
        """Call the executor API to compute HTML samples of auxiliary datasets."""
        self.log_to_file(
            f"Attempting to compute aux dataset sample for paths: {file_paths}, num_rows: {num_rows}"
        )
        try:
            response = requests.post(
            ...
            if "error" in result:
                self.log_to_file(
                    f"Error computing aux dataset sample: {result['error']}"
                )
                return None

            self.log_to_file(
                f"Successfully computed aux dataset sample for paths: {file_paths}"
            )
            return result.get(
                "html_results"
            )  # Expects {'html_results': ['html_string1', ...]}

        except requests.RequestException as e:
            self.log_to_file(
                f"Failed to compute aux dataset sample via API: {str(e)}"
            )
            return None
