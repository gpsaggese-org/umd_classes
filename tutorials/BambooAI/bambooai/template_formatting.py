...
import tutorials.BambooAI.bambooai.template_formatting as tbbatefo
"""
...
class CodeGenPromptGenerator:
    ...
    def format_section(
        self, content: str, formatting_style: str, section_name: str
    ) -> str:
        """
        ...
        needs_yaml = any(
            ...
            for keyword in ["plan", "model", "context"]
        )  # Added 'context'

        if formatting_style == "xml":
            tag = section_name.lower().replace(" ", "_")
            if (
                needs_yaml and content.strip()
            ):  # Ensure content is not just whitespace for yaml
                return f"<{tag}>\n```yaml\n{content}\n```\n</{tag}>"
            else:
                return f"<{tag}>\n{content}\n</{tag}>"
    ...
    def select_template(
        self, analyst: str, planning: bool, model: str, reasoning_models: list
    ) -> str:
        """
        ...
        if analyst == "Data Analyst DF":
            return (
                "code_generator_user_df_plan"
                if use_plan
                else "code_generator_user_df_no_plan"
            )
        else:  # Data Analyst Generic
            return (
                "code_generator_user_gen_plan"
                if use_plan
                else "code_generator_user_gen_no_plan"
            )

    def generate_prompt(
        self,
        generated_datasets_path: str,
        analyst: str,
        planning: bool,
        model: str,
        ...
        template_name = self.select_template(
            analyst, planning, model, reasoning_models
        )
        use_plan = planning and model not in reasoning_models

        # This is the string that instructs the LLM on how to format the path for saving datasets.
        generated_datasets_path_instruction = (
            f"{generated_datasets_path}/<descriptive_name>.csv"
            if generated_datasets_path
            else ""
        )
        ...
        formatted_sections = {
            ...
            "dataframe": self.format_section(
                dataframe_head, formatting_style, "DataFrame"
            ),
            "auxiliary_datasets": self.format_section(
                auxiliary_datasets, formatting_style, "Auxiliary Datasets"
            ),
            "generated_datasets_path_instruction": self.format_section(
                generated_datasets_path_instruction,
                formatting_style,
                "Generated Datasets Path Instruction",
            ),
            "data_model_and_helper_functions": self.format_section(
                data_model, formatting_style, "Data Model and Helper Functions"
            ),
            "task": self.format_section(
                task, formatting_style, "Task"
            ),  # This is the main task description
            "python_version": self.format_section(
                python_version, formatting_style, "Python Version"
            ),
            "pandas_version": self.format_section(
                pandas_version, formatting_style, "Pandas Version"
            ),
            "plotly_version": self.format_section(
                plotly_version, formatting_style, "Plotly Version"
            ),
            "previous_results": self.format_section(
                previous_results, formatting_style, "Previous Results"
            ),
            "example_code": self.format_section(
                example_code, formatting_style, "Example Code"
            ),
        }
        ...
        if template_name == "code_generator_user_df_plan":
            ...
            args = [
                ...
                formatted_sections[
                    "generated_datasets_path_instruction"
                ],  # 4. Generated Datasets Path Instruction
                formatted_sections[
                    "data_model_and_helper_functions"
                ],  # 5. Data Model and Helper Functions
                ...
                formatted_sections[
                    "example_code"
                ],  # 11. Example Code / Final instructions
            ]
            ...
            args = [
                ...
                formatted_sections[
                    "generated_datasets_path_instruction"
                ],  # 8. Generated Datasets Path Instruction
            ]
            ...
            args = [
                ...
                formatted_sections[
                    "generated_datasets_path_instruction"
                ],  # 7. Generated Datasets Path Instruction
            ]
        else:
            # This case should ideally not be reached if select_template is comprehensive
            raise ValueError(
                f"Unknown or unhandled template_name: {template_name}"
            )
