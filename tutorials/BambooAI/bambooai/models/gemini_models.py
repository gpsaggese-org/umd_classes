...
import tutorials.BambooAI.bambooai.models.gemini_models as tbbmgemo
"""
    ...
    for item in messages_copy:
        ...
        try:
                        ...
                        parts.append(
                            types.Part(
                                inline_data=types.Blob(
                                    data=image_bytes, mime_type=part["mime_type"]
                                )
                            )
                        )
            ...
            if parts:
                ...
                message = types.Content(role=item["role"], parts=parts)
...
def llm_call(
    ...
    prompt_tokens = client.models.count_tokens(
        model=model_name, contents=gemini_messages
    ).total_tokens
    ...
    completion_tokens = client.models.count_tokens(
        model=model_name,
        contents=[
            types.ContentDict(role="model", parts=[types.PartDict(text=content)])
        ],
    ).total_tokens
    ...
    try:
        ...
        for chunk in response:
            ...
            for part in candidate.content.parts:
                ...
                has_thought = (
                    hasattr(part, "thought") and part.thought is not None
                )
                ...
                if has_text or has_thought:
                    if has_thought:
                        ...
                        output_manager.print_wrapper(
                            part.text, end="", flush=True, chain_id=chain_id
                        )

            # Capture grounding metadata if present
            if (
                hasattr(chunk.candidates[0], "grounding_metadata")
                ...
                if (
                    ...
                    and metadata.web_search_queries is not None
                ):
                ...
                if (
                    ...
                    for i, q in enumerate(queries):
                        ...
                        end_idx = min(
                            (i + 1) * supports_per_query, len(supports)
                        )
                        relevant_supports = supports[start_idx:end_idx]

                        result_text = " ".join(
                            support.segment.text for support in relevant_supports
                        )
                        relevant_link_indices = set()
                        for support in relevant_supports:
                            relevant_link_indices.update(
                        ...
                        search_triplet.append(
                            {
                                ...
                                "links": [
                                    links[idx]
                                    for idx in relevant_link_indices
                                    if idx < len(links)
                                ],
                            }
                        )
