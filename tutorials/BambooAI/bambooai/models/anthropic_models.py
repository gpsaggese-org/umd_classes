...
import tutorials.BambooAI.bambooai.models.anthropic_models as tbbmanmo
"""
...
def call_and_parse_stream(
    ...
    try:
            ...
            if chunk.type == "content_block_start":
                if chunk.content_block.type == "text":
                    ...
                    output_manager.print_wrapper(
                        content, end="", flush=True, chain_id=chain_id
                    )
                ...
                if (
                    chunk.index == 0 and text_block is not None
                ):  # Check if text_block exists
                    collected_messages.append(full_content)
                    text_block.text = full_content
                elif current_tool_call:
                    tool_calls.append(current_tool_call)
                    tool_use_block.input = json.loads(
                        current_tool_call["function"]["arguments"]
                    )
        ...
        prompt_tokens_used = (
            prompt_tokens_used if "prompt_tokens_used" in locals() else 0
        )
        completion_tokens_used = (
            completion_tokens_used if "completion_tokens_used" in locals() else 0
        )
