...
import tutorials.BambooAI.bambooai.models.local_models as tbbmlomo
"""
...
def convert_openai_to_alpaca(messages: str):
    ...
    formatted_content = re.sub(
        r"Example Output:.*", "", formatted_content, flags=re.S
    )
...
def llm_stream(
    ...
    try:
        ...
        raise ImportError(
            "The torch package is required for using local models. Please install it using pip install torch."
        )

    if cuda.is_available():
        gpu_memory_gb = (
            cuda.get_device_properties(0).total_memory / 1e9
            if cuda.is_available()
            else 0
        )  # In GB
    ...
    if local_model in wizard_coder_models:
        ...
        try:
            ...
            raise ImportError(
                "The auto_gptq package is required for using WizardCoder-GPTQ. Please install it using pip install auto-gptq"
            )
        ...
        error_message = (
            f"Currently the only supported local_models are: {all_models_str}"
        )
        raise ValueError(error_message)

    if (
        local_model in wizard_coder_models
        or local_model in code_llama_instruct_models
        or local_model in code_llama_completion_models
        or local_model in phind_models
        or local_model in open_code_interpreter_models
    ):
        ...
        if cuda.is_available() and gpu_memory_gb >= 80:
            ...
            output_manager.display_system_messages(
                f"Using {model_config['torch_dtype']} precision"
            )
        else:
            model_config = {
            ...
            output_manager.display_system_messages(
                f"Using {model_config['quantization_config'].bnb_4bit_quant_type} quantization"
            )
    ...
    output_manager.display_system_messages(
        f"GPU memory available: {gpu_memory_gb}GB\n"
    )
