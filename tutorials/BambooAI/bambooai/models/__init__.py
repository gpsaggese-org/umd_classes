...
def load_llm_config():
    ...
    sys.exit(
        "Error: LLM_CONFIG.json not found. Please provide model configuration."
    )
...
def init(agent):
    ...
    if not model or not provider:
        raise ValueError(
            f"Agent '{agent}' not found in configuration or has incomplete details"
        )
...
def llm_call(
    log_and_call_manager, messages: str, agent: str = None, chain_id: str = None
):
    ...
    if provider in provider_function_map:
        ...
        (
            ...
            elapsed_time,
            tokens_per_second,
        ) = getattr(provider_module, function_name)(
