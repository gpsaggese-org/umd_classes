...
import tutorials.BambooAI.bambooai.google_search as tbbagose
"""
...
class ChatBot:
    ...
    def execute(
        ...
        self.completion = models.llm_stream(
            prompt_manager,
            log_and_call_manager,
            output_manager,
            messages,
...
class SearchEngine:
    ...
    def search_url(self, url, document_size=CHUNK_SIZE):
        ...
        documents = [
            " ".join(full_words[i : i + document_size])
            for i in range(0, len(full_words), document_size)
        ]
        ...
        if "answerBox" in google_resp and google_resp["answerBox"]:
            ...
            for key, value in google_resp["answerBox"].items():
                if isinstance(value, str) and "https://" in value:
                    ...
                    top_links.append(
                        {
                            "title": google_resp["answerBox"].get(
        ...
        # Check if knowledgeGraph key exists in the response
            ...
            for i, resp in enumerate(google_resp.get("organic", [])):
                ...
                if i < 5:  # Only store top 5 links
                    top_links.append(
                        {"title": resp["title"], "link": resp["link"]}
                    )
...
class Reader:
    def __call__(
        ...
        prompt = prompt_manager.google_search_summarizer_system.format(
            text, query
        )
...
class GeminiSearch:
    ...
    def _parse_response(self, response: str, output_manager, chain_id) -> str:
        ...
        for chunk in metadata.grounding_chunks:
            if chunk.web:
                top_links.append(
                    {"title": chunk.web.title, "link": chunk.web.uri}
                )
            # Check if search_entry_point exists and is not None before accessing rendered_content
            if (
                hasattr(metadata, "search_entry_point")
                and metadata.search_entry_point is not None
                and hasattr(metadata.search_entry_point, "rendered_content")
            ):
    ...
    def __call__(
        ...
        output_manager.display_tool_info(
            "google_ai_search", search_query, chain_id
        )
        response = self._call_gemini(search_query)
        answer, top_links = self._parse_response(
            response, output_manager, chain_id
        )
        return answer, top_links
    ...
    def perform_query(
        ...
        if SEARCH_MODE == "google_ai":
            try:
                ...
                result, links = self.gemini_search(
                    prompt_manager,
                    log_and_call_manager,
                    output_manager,
                    chain_id,
            ...
            while i < max_turns:
                ...
                if actions:
                    ...
                    output_manager.display_tool_info(
                        action, action_input, chain_id
                    )
                    observation, links = self.known_actions[action](
                        prompt_manager,
                        log_and_call_manager,
                        output_manager,
                        chain_id,
                    ...
                    next_prompt.append(
                        {
                            "role": "user",
                            "content": "Observation: {}".format(observation),
                        }
                    )
                else:
                    break
