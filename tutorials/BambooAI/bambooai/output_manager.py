...
import tutorials.BambooAI.bambooai.output_manager as tbbaouma
"""
...
class OutputManager:
    ...
    def display_formated_df(self, df):
        if df is not None:
            if self.is_notebook:
                ...
                cprint(
                    "\n>> Here is the structure of your dataframe:",
                    self.color_result_header_cli,
                    attrs=["bold"],
                )
    ...
    def display_formated_code(self, code):
        if code is not None:
            if self.is_notebook:
                ...
                cprint(
                    "\n>> Here is the final code that accomplishes the task:",
                    self.color_result_header_cli,
                    attrs=["bold"],
                )
    ...
    def display_error(self, error, chain_id=None):
        if self.is_notebook:
            display(
                HTML(
                    f'<br><b><span style="color:{self.color_error_ntb};">I ran into an issue:</span></b><br><pre style="color:{self.color_error_ntb};">{error}</pre><br><b><span style="color:{self.color_error_ntb};">I will examine it, and try again with an adjusted code.</span></b><br>'
                )
            )
        else:
            sys.stderr.write(
                f"{self.color_error_cli['color']}\n>> I ran into an issue:{error}. \n>> I will examine it, and try again with an adjusted code.{self.color_error_cli['reset']}\n"
            )
            sys.stderr.flush()
    ...
    def display_user_input_prompt(self):
        if self.is_notebook:
            display(
                HTML(
                    f"<b style=\"color:{self.color_usr_input_prompt};\">Enter your question or type 'exit' to quit:</b>"
                )
            )
            time.sleep(1)
            question = input()
    ...
    def display_user_input_rank(self):
        if self.is_notebook:
            display(
                HTML(
                    f"<b style=\"color:{self.color_usr_input_rank};\">Are you happy with the ranking ? If YES type 'yes'. If NO type in the new rank on a scale from 1-10:</b>"
                )
            )
            time.sleep(1)
            rank = input()
            ...
            cprint(
                "\nAre you happy with the ranking ?\nIf YES type 'yes'. If NO type in the new rank on a scale from 1-10:",
                self.color_usr_input_rank,
                attrs=["bold"],
            )
            rank = input()
    ...
    def request_user_feedback(
        self, chain_id=None, query_clarification=None, context_needed=None
    ):
        if self.is_notebook:
            display(
                HTML(
                    f'<b style="color:{self.color_usr_input_rank};">Requesting user feedback: "{query_clarification}"</b>'
                )
            )
            time.sleep(1)
            feedback = input()
            ...
            cprint(
                f'\nRequesting user feedback: "{query_clarification}"',
                self.color_usr_input_rank,
            )
            feedback = input()
    ...
    def display_tool_info(self, action, action_input, chain_id=None):
        if self.is_notebook:
            display(
                HTML(
                    f'<span style="color:{self.color_usr_input_rank};">-- Performing Action {action}: "{action_input}"</span>'
                )
            )
            time.sleep(1)
        else:
            cprint(
                f'\n--Performing Action {action}: "{action_input}"',
                self.color_usr_input_rank,
            )
    ...
    def display_system_messages(self, message, chain_id=None):
        if self.is_notebook:
            display(
                HTML(
                    f'<span style="color:{self.color_usr_input_rank};">-- info: "{message}"</span>'
                )
            )
            time.sleep(1)
        else:
