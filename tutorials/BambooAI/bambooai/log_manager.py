...
import tutorials.BambooAI.bambooai.log_manager as tbbaloma
"""
...
class LogAndCallManager:
    def __init__(self, token_cost_dict, user_id: str = None):
        ...
        self.log_dir = (
            os.path.join("logs", self.user_id) if self.user_id else "logs"
        )
        os.makedirs(self.log_dir, exist_ok=True)

        self.run_log_file_path = os.path.join(
            self.log_dir, "bambooai_run_log.json"
        )
        self.consolidated_log_file_path = os.path.join(
            self.log_dir, "consolidated_logs.json"
        )
        ...
        handler = RotatingFileHandler(
            self.consolidated_log_file_path,
    ...
    def print_summary_to_terminal(self, output_manager):
        ...
        summary_text += (
            f"Total Time (LLM Interact.): {elapsed_time:.2f} seconds\n"
        )
        summary_text += (
            f"Average Response Speed: {avg_speed:.2f} tokens/second\n"
        )
    ...
    def write_to_log(
        ...
        cost = ((prompt_tokens * prompt_token_cost) / 1000) + (
            (completion_tokens * completion_token_cost) / 1000
        )
        ...
        with open(self.run_log_file_path, "w") as json_file:
            json.dump(
                existing_json_logs, json_file, indent=2, cls=FlexibleJSONEncoder
            )
    ...
    def consolidate_logs(self):
        ...
        for chain_id, summary_data in self.token_summary.items():
            if chain_id in consolidated_logs:
                summary = {}
                summary["Total LLM Calls"] = len(
                    consolidated_logs[chain_id]["chain_details"]
                )
                ...
                summary["Tokens per Second"] = round(
                    summary_data["completion_tokens"]
                    / summary_data["elapsed_time"],
                    2,
                )
        ...
        with open(self.consolidated_log_file_path, "w") as json_file:
            json.dump(
                consolidated_logs, json_file, indent=2, cls=FlexibleJSONEncoder
            )
