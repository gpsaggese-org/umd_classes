...
import tutorials.BambooAI.bambooai.web_output_manager as tbbwouma
"""
...
class WebOutputManager(OutputManager):
            ...
            if formatted_message:
                if thought:
                    self.output_queue.put(
                        json.dumps(
                            {"thought": formatted_message, "chain_id": chain_id}
                        )
                    )
                else:
                    self.output_queue.put(
                        json.dumps(
                            {"text": formatted_message, "chain_id": chain_id}
                        )
                    )
        else:
            super().print_wrapper(
    ...
    def get_queue_output(self):
        ...
        return "\n".join(
            output
        )  # Join with newlines for compatibility with existing code
    ...
    def request_user_feedback(
        ...
        if self.web_mode:
            ...
            return super().request_user_feedback(
                chain_id, query_clarification, context_needed
            )
        ...
        if self.web_mode:
            if df_id is not None:
                df_index = utils.computeDataframeSample(
                    ...
                    df_id=df_id,
                    executor_client=api_client,
                )
                df_html = df_index.to_html(
                    classes="dataframe", border=0, index=False
                )
                df_json = json.dumps(
                    {"type": "dataframe", "data": df_html, "chain_id": chain_id}
                )
            ...
            for data_type, data in [
                ...
                if data:
                    json_data = json.dumps(
                        {"type": data_type, "data": data, "chain_id": chain_id}
                    )
            ...
            self.output_queue.put(
                json.dumps({"type": "end", "data": None, "chain_id": chain_id})
            )
        else:
    ...
    def display_tool_start(self, agent, model, chain_id=None):
        if self.web_mode:
            self.output_queue.put(
                json.dumps(
                    {
                        "tool_start": {"agent": agent, "model": model},
                        "chain_id": chain_id,
                    }
                )
            )
    ...
    def display_error(self, error, chain_id=None):
        if self.web_mode:
            self.output_queue.put(
                json.dumps({"error": str(error), "chain_id": chain_id})
            )
        else:
    ...
    def display_user_input_rank(self):
        if self.web_mode:
            return (
                None  # In web mode, ranking input is handled by the Flask route
            )
        else:
    ...
    def display_tool_info(self, action, action_input, chain_id=None):
        if self.web_mode:
            self.output_queue.put(
                json.dumps(
                    {
                        "tool_call": {"action": action, "input": action_input},
                        "chain_id": chain_id,
                    }
                )
            )
