...
import tutorials.BambooAI.bambooai.code_executor as tbbacoex
"""
...
class CodeExecutor:
    def __init__(self, webui=False, mode="local", api_client=None, user_id=None):
        ...
        self.mode = mode
        self.plots_dir = (
            os.path.join("iframe_figures", user_id)
            if user_id
            else "iframe_figures"
        )
...
"""
    ...
    def _execute_local(self, code, df=None, generated_datasets_path=None):
        ...
        if generated_datasets_path is not None:
            if not os.path.isdir(generated_datasets_path):
                try:
                    ...
                    self.log_to_file(
                        f"Error creating directory {generated_datasets_path}: {str(e)}"
                    )
        ...
        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    with open(
                                        ...
                                        raw_content = f.read()
                                        # Convert to UTF-8
                                        file_content = raw_content.encode(
                ...
                if os.path.isdir(generated_datasets_path):
                    for filename in os.listdir(generated_datasets_path):
                        file_path = os.path.join(
                            generated_datasets_path, filename
                        )
                    ...
                    if not generated_datasets:
                        try:
                            ...
                            self.log_to_file(
                                f"Error removing empty directory {generated_datasets_path}: {str(e)}"
                            )
                else:
                    self.log_to_file(
                        f"Generated datasets path {generated_datasets_path} does not exist."
                    )
            ...
            exec_traceback = self.filter_exec_traceback(
                code, full_traceback, exc_type.__name__, str(exc_value)
            )
    ...
    def _execute_via_api_client(
        self, code, df=None, df_id=None, generated_datasets_path=None
    ):
    ...
    def execute(self, code, df=None, df_id=None, generated_datasets_path=None):
        ...
        if self.mode == "local":
            ...
            return self._execute_via_api_client(
                code, df, df_id, generated_datasets_path
            )
        else:
    ...
    def filter_exec_traceback(
        self, code, full_traceback, exception_type, exception_value
    ):
        ...
        for line in tb_lines:
            if "<string>" in line:
                line_num = (
                    int(line.split(", line ")[1].split(",")[0]) - patch_offset
                )
        ...
        if error_lines:
            ...
            for i, line in enumerate(
                code_lines[start_line:end_line], start=start_line + 1
            ):
            ...
            for group in traceback_groups:
                for line in group:
                    if "<string>" in line:
                        ...
                        line = line.replace(
                            f"line {original_line_num}",
                            f"line {adjusted_line_num}",
                        )
                        filtered_traceback += line + "\n"
                        if 0 <= adjusted_line_num - 1 < len(code_lines):
                            filtered_traceback += (
                                "    "
                                + code_lines[adjusted_line_num - 1].strip()
                                + "\n"
                            )
