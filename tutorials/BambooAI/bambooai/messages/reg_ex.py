...
import tutorials.BambooAI.bambooai.messages.reg_ex as tbbmreex
"""
...
def _extract_code(response: str, analyst: str, provider: str) -> str:
    ...
    normalized_code_segments = [
        _normalize_indentation(segment) for segment in code_segments
    ]
    ...
    while i < len(lines):
        ...
        if main_start and i == main_start - 1:  # We're at the if __name__ line
            main_content = process_main_block(
                lines, main_start, main_end, main_indent, blacklist
            )
...
def _remove_examples(messages: str) -> str:
    ...
    for dict in messages:
        ...
        if dict.get("role") == "user" and "content" in dict:
            dict["content"] = re.sub(
                pattern, "", dict["content"], flags=re.DOTALL
            )
