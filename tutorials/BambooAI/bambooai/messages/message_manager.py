...
import tutorials.BambooAI.bambooai.messages.message_manager as tbbmmema
"""
...
class MessageManager:
    def __init__(
        ...
        try:
            ...
            self.output_manager.print_wrapper(
                f"Warning: Failed to initialize interaction store: {e}"
            )
    ...
    def store_interaction(
        ...
        if self.interaction_store:
            ...
            tool_results = {
                ...
                if search_results["search"]["searches"]
                else None,
                "code_exec": {
    ...
    def reset_messages(self, prompts):
        self.pre_eval_messages = [
            {"role": "system", "content": prompts.expert_selector_system}
        ]
        self.select_analyst_messages = [
            {"role": "system", "content": prompts.analyst_selector_system}
        ]
        self.eval_messages = [
            {"role": "system", "content": prompts.planner_system}
        ]
        self.code_messages = [
            {"role": "system", "content": prompts.code_generator_system_df}
        ]
        self.df_inspector_messages = [
            {"role": "system", "content": prompts.dataframe_inspector_system}
        ]
        self.code_exec_results = None
    ...
    def messages_maintenace(self, messages: list):
        ...
        for i in range(
            len(messages) - 1, -1, -1
        ):  # Start from the last item to index 0
            msg = messages[i]
    ...
    def append_qa_pair(self, question, results):
        ...
        self.qa_pairs = [
            ...
            if not any(
                identifier in pair["task"] for identifier in custom_identifiers
            )
        ]
    ...
    def format_qa_pairs(self, max_qa_pairs=8):
        ...
        for i, pair in enumerate(self.qa_pairs, 1):
            ...
            answer_lines = [
                line for line in pair["result"].split("\n") if line.strip()
            ]
    ...
    def messages_content_maintenance(
        self, agent, messages, model_template_formatting
    ):
        ...
        if agent == "Dataframe Inspector":
            _process_user_messages(
                messages, reg_ex._remove_all_except_task_ontology_text
            )
            ...
            regex_pattern = (
                reg_ex._remove_all_except_task_xml
                if model_template_formatting == "xml"
                else reg_ex._remove_all_except_task_text
            )
            _process_user_messages(messages, regex_pattern)

    def format_image_message(
        self, agent, message_content, image, provider, model
    ):
