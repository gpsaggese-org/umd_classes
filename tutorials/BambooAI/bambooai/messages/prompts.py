...
import tutorials.BambooAI.bambooai.messages.prompts as tbbamepr
"""
...
class PromptManager:
    def _load_yaml_file(
        self, package_path, filename, is_default=True, direct_path=None
    ):
        data = {}
        try:
            if is_default:
                with importlib.resources.open_text(
                    package_path, filename, encoding="utf-8"
                ) as f:
                ...
                not is_default and direct_path
            ):  # Custom file specified but not found
                warnings.warn(
                    f"Custom prompt file specified but not found: {direct_path}",
                    UserWarning,
                )
            ...
            if is_default:
                warnings.warn(
                    ...
                    "Prompts relying on defaults may be empty.",
                    UserWarning,
                )
        except yaml.YAMLError as e:
            file_in_error = filename if is_default else direct_path
            warnings.warn(
                f"Error parsing YAML file '{file_in_error}': {e}. Check its format.",
                UserWarning,
            )
        except Exception as e:
            file_in_error = filename if is_default else direct_path
            warnings.warn(
                f"Unexpected error loading YAML file '{file_in_error}': {e}",
                UserWarning,
            )
        return data
    ...
    def _load_all_prompts(self):
        ...
        default_prompts_data = self._load_yaml_file(
            _DEFAULT_PROMPTS_PACKAGE_PATH,
            _DEFAULT_PROMPTS_FILENAME,
            is_default=True,
        )
        ...
        if self.effective_custom_prompts_file:
            custom_prompts_data = self._load_yaml_file(
                ...
                None,
                is_default=False,
                direct_path=self.effective_custom_prompts_file,
            )
            ...
            if not isinstance(value, str):
                if value is not None:
                    warnings.warn(
                        ...
                        f"Using empty string. Value: {str(value)[:50]}...",
                        UserWarning,
                    )
                value = ""
