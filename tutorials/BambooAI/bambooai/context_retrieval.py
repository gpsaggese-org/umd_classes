...
import tutorials.BambooAI.bambooai.context_retrieval as tbbacore
"""
...
def request_user_context(
    ...
    output_manager.display_tool_info(
        "Feedback Request",
        "The model needs clarification on your query",
        chain_id=chain_id,
    )
    ...
    feedback_file = (
        os.path.join("temp", user_id, f"feedback_{chain_id}.json")
        if user_id
        else os.path.join("temp", f"feedback_{chain_id}.json")
    )
        ...
        if os.path.exists(feedback_file):
            try:
                ...
                for feedback_entry in feedback_list:
                    if (
                        feedback_entry["query_clarification"]
                        == query_clarification
                    ):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to delete {feedback_file}: {str(e)}"
                            )
                        return feedback
            except (json.JSONDecodeError, KeyError, IOError) as e:
                logger.warning(
                    f"Error reading {feedback_file}: {str(e)}. Continuing to poll."
                )
        else:
