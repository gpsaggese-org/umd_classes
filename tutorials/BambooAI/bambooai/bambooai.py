...
import tutorials.BambooAI.bambooai.bambooai as tbababam
"""
...
class BambooAI:
    def __init__(
        ...
        # Output
        self.output_manager = (
            web_output_manager.WebOutputManager()
            if webui
            else output_manager.OutputManager()
        )
        self.webui = webui
        ...
        self.api_client = executor_client.ExecutorAPIClient(
            base_url=self.executor_api_url
        )
        ...
        if vector_db:
            ...
            )  # Default to pinecone for backward compatibility
            ...
            if vector_db_type.lower() == "pinecone":
                ...
                if PINECONE_API_KEY is not None:
                    ...
                    self.output_manager.display_system_messages(
                        "Warning: PINECONE_API_KEY environment variable not found. Disabling vector_db."
                    )
                    vector_db = False

            elif vector_db_type.lower() == "qdrant":
                try:
                    self.qdrant_wrapper = qa_retrieval.QdrantWrapper(
                        output_manager=self.output_manager
                    )
                    self.vector_db_wrapper = self.qdrant_wrapper
                except Exception as e:
                    self.output_manager.display_system_messages(
                        f"Error initializing Qdrant: {str(e)}"
                    )
                    vector_db = False

            else:
                self.output_manager.display_system_messages(
                    f"Warning: Unsupported vector database type '{vector_db_type}'. Supported types: 'pinecone', 'qdrant'. Disabling vector_db."
                )
                vector_db = False
        ...
        self.original_df_columns = (
            utils.get_dataframe_columns(
                df, self.execution_mode, df_id, self.api_client
            )
        ...
        self.auxiliary_datasets = (
            auxiliary_datasets if auxiliary_datasets is not None else []
        )
        ...
        self.code_gen_prompt_generator = (
            template_formatting.CodeGenPromptGenerator(
                code_gen_template_dict, self.model_dict
            )
        )
    ...
    def select_expert(self, pre_eval_messages):
        ...
        self.output_manager.display_tool_start(
            agent, using_model, chain_id=self.chain_id
        )
        ...
        expert, requires_dataset, confidence = reg_ex._extract_expert(
            llm_response
        )
    ...
    def select_analyst(self, select_analyst_messages):
        ...
        if provider in ["openai", "anthropic", "gemini", "groq"]:
            tools = tools_definition.filter_tools(
                provider,
                search_enabled=False,
                feedback_enabled=self.user_feedback,
            )
        else:
            tools = None
        ...
        if tools:
            llm_response, tool_response = self.llm_stream(
                self.prompts,
                self.log_and_call_manager,
                self.output_manager,
        ...
        analyst, query_unknown, query_condition, data_descr, intent_breakdown = (
            reg_ex._extract_analyst(llm_response)
        )

        # Retrieve the matching data_model, code and plan from the vector database if exists
        if self.vector_db:
            self.output_manager.print_wrapper(
                "I am now going to search the episodic memory for similar tasks to the current one. if I find a match, I will use the plan, data model and code from the previous task to help me with the current task.",
                chain_id=self.chain_id,
            )
            self.output_manager.display_tool_info(
                "Semantic Search",
                "Searching episodic memory for similar tasks",
                chain_id=self.chain_id,
            )
            vector_data = self.vector_db_wrapper.retrieve_matching_record(
                intent_breakdown,
                data_descr,
                similarity_threshold=self.similarity_threshold,
            )
            if vector_data:
                self.retrieved_similarity_score = str(
                    round(vector_data["score"] * 100, 1)
                )
                ...
                self.output_manager.display_results(
                    chain_id=self.chain_id, semantic_search=None
                )
        else:
            self.output_manager.print_wrapper(
    ...
    def task_eval(self, eval_messages, agent, image=None):
        ...
        if provider in ["openai", "anthropic", "gemini", "groq"]:
            tools = tools_definition.filter_tools(
                provider,
                search_enabled=self.search_tool,
                feedback_enabled=self.user_feedback,
            )
        else:
            tools = None
    ...
    def taskmaster(self, question, df_summary, aux_datasets_columns, image=None):
        ...
        self.message_manager.pre_eval_messages.append(
            {"role": "user", "content": template}
        )
        select_expert_llm_response, expert, requires_dataset, confidence = (
            self.select_expert(self.message_manager.pre_eval_messages)
        )
        self.message_manager.pre_eval_messages.append(
            {"role": "assistant", "content": select_expert_llm_response}
        )

        ######## Refine Expert Selection, and Formulate the task for the expert ###########
        if expert == "Data Analyst":
            self.message_manager.select_analyst_messages.append(
                {
                    "role": "user",
                    "content": self.prompts.analyst_selector_user.format(
            ...
            (
                select_analyst_llm_response,
                analyst,
                query_unknown,
                query_condition,
                data_descr,
                intent_breakdown,
            ) = self.select_analyst(self.message_manager.select_analyst_messages)
            self.user_intent = intent_breakdown
            self.message_manager.select_analyst_messages.append(
                {"role": "assistant", "content": select_analyst_llm_response}
            )
            ...
            self.message_manager.messages_maintenace(
                self.message_manager.select_analyst_messages
            )
            self.message_manager.messages_content_maintenance(
                "Analyst Selector",
                self.message_manager.select_analyst_messages,
                self.model_dict[models.get_model_name("Code Generator")[0]][
                    "templ_formating"
                ],
            )

            if self.retrieved_plan is not None:
                example_plan = self.prompts.semantic_memory_plan_example.format(
                    self.retrieved_similarity_score,
                    self.retrieved_rank,
                    self.retrieved_plan,
                )
            else:
            ...
            if analyst == "Data Analyst DF":
                if (
                    self.df_ontology and self.retrieved_data_model is None
                ):  # Only inspect the dataframe using ontology if the data_model is not retrieved from the vector database
                    query = intent_breakdown
                    data_model, df_inspector_messages = utils.inspect_dataframe(
                        df=self.df,
                        prompt_manager=self.prompts,
                        log_and_call_manager=self.log_and_call_manager,
                        output_manager=self.output_manager,
                        chain_id=self.chain_id,
                        query=query,
                        execution_mode=self.execution_mode,
                        df_ontology=self.df_ontology,
                        df_id=self.df_id,
                        aux_file_paths=self.auxiliary_datasets,
                        executor_client=self.api_client,
                        messages=self.message_manager.df_inspector_messages,
                    )
                    if df_inspector_messages:
                        self.message_manager.df_inspector_messages = (
                            df_inspector_messages
                        )
                    self.data_model = reg_ex._extract_data_model(data_model)
                    dataframe_head = utils.inspect_dataframe(
                        df=self.df,
                        execution_mode=self.execution_mode,
                        df_id=self.df_id,
                        executor_client=self.api_client,
                    ...
                    self.message_manager.messages_content_maintenance(
                        ...
                        self.message_manager.df_inspector_messages,
                        self.model_dict[
                            models.get_model_name("Code Generator")[0]
                        ]["templ_formating"],
                    )
                    ...
                    self.output_manager.display_results(
                        chain_id=self.chain_id, data_model=data_model_web
                    )
                elif (
                    self.df_ontology and self.retrieved_data_model is not None
                ):  # Use the data_model retrieved from the vector database
                ...
                if (
                    models.get_model_name("Planner")[0]
                    not in self.reasoning_models
                ):
                    self.message_manager.eval_messages.append(
                        {
                            "role": "user",
                            "content": self.prompts.planner_user_df.format(
                                utils.get_readable_date(),
                                self.message_manager.format_qa_pairs(),
                                intent_breakdown,
                                None if self.df_id is None else dataframe_head,
                                utils.aux_datasets_to_string(
                                    file_paths=self.auxiliary_datasets,
                                    execution_mode=self.execution_mode,
                                    executor_client=self.api_client,
                                ),
                                f"```yaml\n{self.data_model}\n```",
                    ...
                    self.message_manager.eval_messages.append(
                        {
                            "role": "user",
                            "content": self.prompts.planner_user_df_reasoning.format(
                                utils.get_readable_date(),
                                self.message_manager.format_qa_pairs(),
                                intent_breakdown,
                                None if self.df_id is None else dataframe_head,
                ...
                if not self.planning:
                    self.message_manager.eval_messages.append(
                        {
                            "role": "assistant",
                            "content": "No content, as planning was disabled for this task",
                        }
                    )  # We just add a dummy assistant message if planning is disabled, to maintain the conversation messages structure

                # Replace first dict in messages with a new system task. This is to distinguish between the two types of analysts
                self.message_manager.code_messages[0] = {
                    "role": "system",
                    "content": self.prompts.code_generator_system_df,
                }
                ...
                if (
                    ...
                    self.message_manager.eval_messages.append(
                        {
                            "role": "user",
                            "content": self.prompts.planner_user_gen.format(
                                utils.get_readable_date(),
                                self.message_manager.format_qa_pairs(),
                                intent_breakdown,
                                example_plan,
                    ...
                    self.message_manager.eval_messages.append(
                        {
                            "role": "user",
                            "content": self.prompts.planner_user_gen_reasoning.format(
                                utils.get_readable_date(),
                                self.message_manager.format_qa_pairs(),
                                intent_breakdown,
                            ),
                ...
                self.message_manager.code_messages[0] = {
                    "role": "system",
                    "content": self.prompts.code_generator_system_gen,
                }
            agent = "Planner"
            ...
            self.message_manager.eval_messages.append(
                {
                    "role": "user",
                    "content": self.prompts.theorist_system.format(
                        utils.get_readable_date(),
                        None if self.df_id is None else df_summary,
                        aux_datasets_columns,
        ...
        if (
            ...
            or (
                self.planning
                and models.get_model_name("Code Generator")[0]
                not in self.reasoning_models
            )
        ):  # If the analyst is not selected or the code generator model is not a reasoning model
            task_eval, tool_response, full_response = self.task_eval(
                self.message_manager.eval_messages, agent, image
            )
            self.message_manager.eval_messages.append(
        ...
        self.message_manager.messages_maintenace(
            self.message_manager.eval_messages
        )
        self.message_manager.messages_content_maintenance(
            agent,
            self.message_manager.eval_messages,
    ...
    def _process_question(self, question, image, user_code):
        if self.webui:
            self.output_manager.send_chain_id(
                self.thread_id, self.chain_id, self.df_id
            )  # Send the thread_id and chain_id for the new chain to the web interface

        # Path to where the generated datasets will be stored.
        generated_datasets_path = os.path.join(
            "datasets",
            self.user_id or "",
            "generated",
            str(self.thread_id),
        ...
        if user_code is None:
            if self.exploratory is True:
                ...
                (
                    ...
                    else utils.dataframe_summary_to_string(
                        self.df, self.execution_mode, self.df_id, self.api_client
                    ),
                    utils.get_aux_datasets_columns(
                        file_paths=self.auxiliary_datasets,
                        execution_mode=self.execution_mode,
                        executor_client=self.api_client,
                    ),
                ...
                if not analyst:
                    ...
                    self.message_manager.store_interaction(
                        self.thread_id,
                        self.chain_id,
                        google_search_results=tool_response,
                        code_exec_results=self.message_manager.code_exec_results,
                        executed_code=self.message_manager.last_code,
                        qa_pairs=self.message_manager.qa_pairs,
                        tasks=self.message_manager.tasks,
                    )
                    return
                else:
                    if (
                        ...
                        # Create a dictionary containing both the visualization and the YAML data
                        plan_web = {"visualization": plan_vis, "yaml": plan}
                        self.output_manager.display_results(
                            chain_id=self.chain_id,
                            research=tool_response,
                            plan=plan_web,
            ...
            if self.vector_db:
                if self.retrieved_code is not None:
                    example_code = (
                        self.prompts.semantic_memory_code_example.format(
                            self.retrieved_similarity_score,
                            self.retrieved_rank,
                            self.retrieved_code,
                        )
            ...
            if code is None or code == "":
                self.output_manager.display_results(
                    chain_id=self.chain_id, answer=llm_response
                )
                self.log_and_call_manager.print_summary_to_terminal(
                    self.output_manager
                )
        ...
        answer, results, code, plot_jsons, error_corrections = self.execute_code(
            analyst,
            code,
            plan,
            intent_breakdown,
        ...
        if plan:
            if (
                ...
                reviewed_plan = (
                    self.review_plan(code, plan)
                    if (self.vector_db and error_corrections > 0)
                    else plan
                )
        ...
        if user_code is None:
            if self.webui:
                ...
                self.output_manager.output_queue.put(
                    json.dumps({"rank_data": rank_data})
                )
            else:
                if self.vector_db:
    ...
    def pd_agent_converse(
        ...
        if chain_id is not None and str(chain_id) != str(self.chain_id):
            try:
                self.message_manager.restore_interaction(
                    self.thread_id, chain_id
                )
            except storage_manager.StorageError as e:
                self.output_manager.print_wrapper(
                    f"Warning: Failed to restore chain {chain_id}: {e}"
                )
    ...
    def generate_code(
        ...
        if analyst == "Data Analyst DF":
            ...
            data_model = (
                f"{self.data_model}"
                if self.df_ontology and self.data_model is not None
                else None
            )
        else:
            dataframe_head = None
        ...
        formatted_prompt = self.code_gen_prompt_generator.generate_prompt(
            ...
            auxiliary_datasets=utils.aux_datasets_to_string(
                file_paths=self.auxiliary_datasets,
                execution_mode=self.execution_mode,
                executor_client=self.api_client,
            ),
    ...
    def execute_code(
        ...
        while error_corrections < self.MAX_ERROR_CORRECTIONS:
            ...
            if code is not None:
                self.output_manager.display_tool_info(
                    "Code Execution",
                    f"exec(code,'df': pd.DataFrame) in {execution_mode} mode",
                    chain_id=self.chain_id,
                )
                ...
                (
                    ...
                    code, self.df, self.df_id, generated_datasets_path
                )

                if error:
                    error_corrections += 1
                    code, code_messages = self.correct_code_errors(
                        error,
                        error_corrections,
                        code_messages,
                        analyst,
                        code_type,
                        code if code_type == "user" else None,
                    )
        ...
        if self.webui:
            ...
            self.output_manager.display_results(
                chain_id=self.chain_id, code_exec_results=results
            )
        ...
        self.output_manager.display_results(
            chain_id=self.chain_id,
            code=code,
            plot_jsons=plot_jsons if self.webui else None,
            generated_datasets=generated_datasets
            if generated_datasets
            else None,
        )
        summary = self.summarise_solution(
            intent_breakdown,
            plan,
            results,
            code if code_type == "user" else None,
        ...
        return (
            ...
            results,
            code,
            plot_jsons if self.webui else None,
            error_corrections,
        )
        ...
        if code_type == "user" and error_corrections == 1:
            if self.model_dict[model]["templ_formating"] == "xml":
                code_messages.append(
                    {
                        "role": "user",
                        "content": self.prompts.error_corector_edited_system.format(
                            code,
                            error,
                            self.python_version,
                            self.pandas_version,
                ...
                code_messages.append(
                    {
                        "role": "user",
                        "content": self.prompts.error_corector_edited_system_reasoning.format(
                            code,
                            error,
                            self.python_version,
                            self.pandas_version,
            ...
            if self.model_dict[model]["templ_formating"] == "xml":
                code_messages.append(
                    {
                        "role": "user",
                        "content": self.prompts.error_corector_system.format(
                            error,
                            self.python_version,
                            self.pandas_version,
                            self.plotly_version,
                ...
                code_messages.append(
                    {
                        "role": "user",
                        "content": self.prompts.error_corector_system_reasoning.format(
                            error,
                            self.python_version,
                            self.pandas_version,
                            self.plotly_version,
    ...
    def review_plan(self, code, plan):
        ...
        self.message_manager.plan_review_messages = [
            {
                "role": "user",
                "content": self.prompts.reviewer_system.format(code, plan),
            }
        ]
        ...
        if provider == "gemini":
            ...
            reasoning_effort = (
                "low"  # Minimal is not supported by OpenAI and Anthropic
            )
        ...
        self.message_manager.plan_review_messages.append(
        ...
        plan_vis = utils.generate_plan_graph(plan)
        # Create a dictionary containing both the visualization and the YAML data
        plan_web = {"visualization": plan_vis, "yaml": reviewed_plan}
        self.output_manager.display_results(
            chain_id=self.chain_id, plan=plan_web
        )
    ...
    def summarise_solution(self, intent_breakdown, plan, results, code=None):
        ...
        # Initialize the messages list with a user message containing the task prompt
        if code is not None:
            self.message_manager.insight_messages = [
                {
                    "role": "user",
                    "content": self.prompts.solution_summarizer_custom_code_system.format(
                        code, results
                    ),
                }
            ]
            ...
            self.message_manager.insight_messages = [
                {
                    "role": "user",
                    "content": self.prompts.solution_summarizer_system.format(
                        intent_breakdown, plan, results
                    ),
                }
            ]
        ...
        self.message_manager.insight_messages.append(
            {"role": "assistant", "content": summary}
        )

        self.output_manager.display_results(
            chain_id=self.chain_id, answer=summary
        )
