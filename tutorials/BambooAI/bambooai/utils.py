...
import tutorials.BambooAI.bambooai.utils as tbabauti
"""
...
def get_package_versions():
    ...
    # Check for pandas
    versions["pandas_version"] = installed_packages.get(
        "pandas", "Not installed"
    )

    # Check for plotly
    versions["plotly_version"] = installed_packages.get(
...
def dataframe_summary_to_string(
    df: pd.DataFrame,
    ...
    for col in df.columns:
        ...
        if pd.api.types.is_numeric_dtype(df[col]):
            ...
            if len(vals) > 0:
                result.append(
                    f"{col}: numeric(n={len(vals)}) range={vals.min():.1f}-{vals.max():.1f} mean={vals.mean():.1f}{missing_info}"
                )
            else:
            ...
            result.append(
                f"{col}: categorical(n={df[col].count()}) unique={unique_count}{samples}{missing_info}"
            )
...
def dataframe_to_string(
    ...
    try:
        ...
        # Get summary string
        summary_string = dataframe_summary_to_string(
            df, execution_mode, df_id, executor_client
        )
...
def aux_datasets_to_string(
    ...
    if execution_mode == "api" and executor_client is not None:
        api_result = executor_client.aux_datasets_to_string(
            ...
            if file_ext == ".csv":
                ...
                if parquet_file.num_row_groups > 0:
                    df = parquet_file.read_row_group(
                        0, columns=parquet_file.schema.names
                    ).to_pandas()
...
def get_aux_datasets_columns(
    file_paths: List[str], execution_mode: str = "local", executor_client=None
) -> str:
    """
    ...
    """
    if execution_mode == "api" and executor_client is not None:
        api_result = executor_client.get_aux_datasets_columns(
...
def compute_aux_dataset_sample(
    file_paths: List[str],
    num_rows: int = 100,
    execution_mode: str = "local",
    executor_client=None,
) -> List[str]:  # Returns list of HTML strings
    """
    Load a sample of each dataset in file_paths and convert to HTML
    in a memory-efficient way, either locally or via executor API.
    """
    if execution_mode == "api" and executor_client is not None:
        api_result = executor_client.compute_aux_dataset_sample(
            file_paths=file_paths, num_rows=num_rows
        )
        if api_result is not None:
            return api_result
    ...
    if not file_paths:  # Handle empty file_paths list
        error_df = pd.DataFrame(
            [{"Error": "No auxiliary dataset paths provided."}]
        )
        html_results.append(
            error_df.to_html(classes="dataframe", border=0, index=False)
        )
        return html_results
    ...
    for path in file_paths:
        ...
        try:
            if not os.path.exists(path):
                df = pd.DataFrame(
                    [{"Error": f"File not found: {os.path.basename(path)}"}]
                )
            elif file_ext == ".csv":
                df = pd.read_csv(path, nrows=num_rows)
            elif file_ext in [".parquet", ".pq"]:
                parquet_file = pq.ParquetFile(path)
                if parquet_file.num_row_groups > 0:
                    first_row_group_reader = parquet_file.reader.read_row_group(
                        0
                    )
                    ...
                    df = pd.DataFrame(
                        [
                            {
                                "Info": f"Parquet file is empty: {os.path.basename(path)}"
                            }
                        ]
                    )
            else:
            ...
            error_df = pd.DataFrame(
                [
                    {
                        "Error": f"Failed to process {os.path.basename(path)}: {str(e)}"
                    }
                ]
            )
            html_results.append(
...
def inspect_dataframe(
    ...
    if log_and_call_manager:
        try:
            ...
            primary_df_head = dataframe_to_string(
                df=df,
                execution_mode=execution_mode,
                df_id=df_id,
                executor_client=executor_client,
            )
            auxiliary_datasets_heads = aux_datasets_to_string(
                file_paths=aux_file_paths,
                execution_mode=execution_mode,
                executor_client=executor_client,
            )
            ...
            output_manager.display_tool_start(
                agent, models.get_model_name(agent)[0], chain_id
            )
            ...
            llm_response = models.llm_stream(
                prompt_manager,
            ...
            if llm_response:
                df_inspector_messages.append(
                    {"role": "assistant", "content": llm_response}
                )

            return llm_response, df_inspector_messages
        except Exception:
            output_manager.display_system_messages("Error generating Data Model")
            raise
    else:
        return dataframe_to_string(
            df=df,
            execution_mode=execution_mode,
            df_id=df_id,
            executor_client=executor_client,
...
def generate_model_graph(yaml_string):
    def create_node_label(node_data, node_name_override=None):
        ...
        primary_name = (
            ...
            else node_data.get("name", "Unnamed")
        )
        if not primary_name:
            primary_name = "Unnamed_Node_In_Label"
            # This warning is useful if it happens
            logging.warning(
                f"Node data missing 'name', using '{primary_name}' in label. Data: {str(node_data)[:100]}..."
            )
        ...
        for key, display_name in props_to_include_map.items():
            if node_data.get(key):
                ...
                if len(prop_value) > 40 and key not in [
                    "role_in_grouping",
                    "derivation_method",
                ]:
    ...
    def generate_mermaid_hierarchical(data):
        ...
        def define_node(original_name, display_label_str, node_type_hint):
            if not original_name:  # Should be a string
                logging.error(
                    f"Attempted to define a node with no/empty original_name. Label: '{display_label_str[:50]}...', Type: '{node_type_hint}'. Skipping node."
                )
                return None
            ...
            if original_name not in node_ids:
                ...
                mermaid_code.append(
                    f"    style {node_id} {get_node_style(style_type)}"
                )
            return node_id
        ...
        node_definition_sources = [
            (
                data.get("data_hierarchy", []),
                lambda item: item.get("type", "container"),
            ),
            (data.get("keys", []), lambda item: "key"),
            (
                data.get("measurements_attributes", []),
                lambda item: "measurement",
            ),
            (
                data.get("components_sub_entities", []),
                lambda item: item.get("type", "component"),
            ),
        ]
        ...
        for item_list, type_extractor_func in node_definition_sources:
            for item_data in item_list:
                if not isinstance(item_data, dict):
                    logging.warning(
                        f"Skipping non-dictionary item in node definition source: {str(item_data)[:100]}..."
                    )
                    continue
                node_name = item_data.get("name")
                if (
                    not isinstance(node_name, str) or not node_name
                ):  # Ensure name is a non-empty string
                    logging.warning(
                        f"Item missing 'name' or name is not a string, cannot define node: {str(item_data)[:100]}..."
                    )
                    continue
                ...
                if "derived_objects" in item_data and item_list is data.get(
                    "data_hierarchy", []
                ):  # Check if item_list is indeed data_hierarchy
                    for derived_obj_data in item_data.get("derived_objects", []):
                        if not isinstance(derived_obj_data, dict):
                            logging.warning(
                                f"Skipping non-dictionary derived_object: {str(derived_obj_data)[:100]}..."
                            )
                            continue
                        derived_name = derived_obj_data.get("name")
                        if not isinstance(derived_name, str) or not derived_name:
                            logging.warning(
                                f"Derived object missing 'name' or name is not a string: {str(derived_obj_data)[:100]}..."
                            )
                            continue
                        derived_type = derived_obj_data.get(
                            "type", "derived_object"
                        )
                        derived_label_str = create_node_label(derived_obj_data)
                        define_node(
                            derived_name, derived_label_str, derived_type
                        )
        ...
        if "functions" in data:
            for function_data in data.get("functions", []):
                if not isinstance(function_data, dict):
                    logging.warning(
                        f"Skipping non-dictionary function item: {str(function_data)[:100]}..."
                    )
                    continue
                func_name = function_data.get("name")
                if not isinstance(func_name, str) or not func_name:
                    logging.warning(
                        f"Function item missing 'name' or name is not a string: {str(function_data)[:100]}..."
                    )
                    continue
                func_label_parts = [f"<b>{func_name}</b>"]
                if function_data.get("rdfs_comment"):
                    comment = str(function_data["rdfs_comment"])
                    func_label_parts.append(
                        f"Desc: {comment[:60] + '...' if len(comment) > 60 else comment}"
                    )
        ...
        if "data_hierarchy" in data:
            for item_data in data.get("data_hierarchy", []):
                if not isinstance(item_data, dict) or not item_data.get("name"):
                    continue
                domain_title = item_data.get(
                    "domain_label", "Uncategorized_Data"
                )
                if not isinstance(domain_title, str):
                    domain_title = str(
                        domain_title
                    )  # Ensure string for sanitize_id
                ...
                if "derived_objects" in item_data:
                    for derived_obj_data in item_data.get("derived_objects", []):
                        if (
                            isinstance(derived_obj_data, dict)
                            and derived_obj_data.get("name") in node_ids
                        ):
                            derived_node_id = node_ids[derived_obj_data["name"]]
                            if (
                                derived_node_id
                                not in domain_groups[domain_title]
                            ):
                                domain_groups[domain_title].append(
                                    derived_node_id
                                )

        if "functions" in data and any(
            isinstance(f, dict) and f.get("name") in node_ids
            ...
            for function_data in data.get("functions", []):
                if (
                    ...
                    and function_data.get("name") in node_ids
                ):
                    mermaid_code.append(
                        f"        {node_ids[function_data['name']]}"
                    )
            mermaid_code.append("    end")
            mermaid_code.append("    class Functions_Domain subgraphStyle;")

        for domain_title, nodes_in_domain_group in domain_groups.items():
            subgraph_id = sanitize_id(f"{domain_title}_Domain")
            formatted_subgraph_title = (
                format_label(domain_title, max_width=30)
                .replace("<br/>", " ")
                .replace("<br>", " ")
            )
            mermaid_code.append(
                f"subgraph {subgraph_id} [{formatted_subgraph_title}]"
            )
        ...
        def add_relationship(
            ...
            if not (
                isinstance(source_original_name, str) and source_original_name
            ) or not (
                isinstance(target_original_name, str) and target_original_name
            ):
                logging.warning(
                    f"Skipping relationship '{relationship_label}' due to invalid/empty source/target name. Source='{source_original_name}', Target='{target_original_name}'"
                )
                return

            if (
                source_original_name in node_ids
                and target_original_name in node_ids
            ):
                source_id = node_ids[source_original_name]
                target_id = node_ids[target_original_name]

                rel_display_label = str(
                    relationship_label
                )  # Ensure it's a string
                rel_display = sanitize_id(rel_display_label).replace("_", " ")
                if not rel_display:
                    rel_display = (
                        "related"  # Fallback for empty label after sanitize
                    )
                ...
                if relationship_signature not in added_relationships:
                    ...
                    mermaid_code.append(
                        f"    {source_id} {arrow}|{rel_display}| {target_id}"
                    )
                ...
                logging.warning(
                    f"Cannot draw relationship '{relationship_label}' between '{source_original_name}' and '{target_original_name}'. One or both nodes not found in defined node_ids. Source defined: {source_original_name in node_ids}, Target defined: {target_original_name in node_ids}"
                )
        ...
        if "data_hierarchy" in data:
            for item_data in data.get("data_hierarchy", []):
                ...
                if item_data.get("contains"):
                    ...
                    if not isinstance(contained_items_refs, list):
                        contained_items_refs = [contained_items_refs]
                    for contained_ref in contained_items_refs:
                        contained_name = (
                            contained_ref
                            if isinstance(contained_ref, str)
                            else (
                                contained_ref.get("name")
                                if isinstance(contained_ref, dict)
                                else None
                            )
                        )
                        if contained_name:  # Ensure contained_name is valid
                            add_relationship(
                                parent_name, contained_name, "contains"
                            )
                if "derived_objects" in item_data:
                    for derived_obj_data in item_data.get("derived_objects", []):
                        if isinstance(
                            derived_obj_data, dict
                        ) and derived_obj_data.get("name"):
                            derived_name = derived_obj_data["name"]
                            add_relationship(
                                parent_name, derived_name, "derives"
                            )
                            comp_func_refs = derived_obj_data.get(
                                "canBeComputedUsingFunction"
                            )
                            if comp_func_refs:
                                if not isinstance(comp_func_refs, list):
                                    comp_func_refs = [comp_func_refs]
                                for func_name_ref in comp_func_refs:
                                    if (
                                        isinstance(func_name_ref, str)
                                        and func_name_ref
                                    ):
                                        add_relationship(
                                            func_name_ref,
        ...
        if "keys" in data:
            for key_data in data.get("keys", []):
                if not isinstance(key_data, dict) or not key_data.get("name"):
                    continue
                ...
                if isinstance(associated_obj_ref, str) and associated_obj_ref:
                    ...
                    for assoc_obj_name_item in associated_obj_ref:
                        if (
                            isinstance(assoc_obj_name_item, str)
                            and assoc_obj_name_item
                        ):
                            add_relationship(
                                assoc_obj_name_item, key_name, "has key"
                            )

                key_relations_ref = key_data.get("hasRelation")
                if key_relations_ref:
                    if not isinstance(key_relations_ref, list):
                        key_relations_ref = [key_relations_ref]
                    for related_key_name_item in key_relations_ref:
                        if (
                            isinstance(related_key_name_item, str)
                            and related_key_name_item
                        ):
                            add_relationship(
                                key_name, related_key_name_item, "has relation"
                            )
        ...
        if "measurements_attributes" in data:
            for measurement_data in data.get("measurements_attributes", []):
                ...
                ) or not measurement_data.get("name"):
                    continue
                ...
                if associated_objs_list:
                    if not isinstance(associated_objs_list, list):
                        associated_objs_list = [associated_objs_list]
        ...
        if "functions" in data:
            for function_data in data.get("functions", []):
                if not isinstance(function_data, dict) or not function_data.get(
                    "name"
                ):
                    continue
                ...
                if applies_to_objs_ref:
                    if not isinstance(applies_to_objs_ref, list):
                        applies_to_objs_ref = [applies_to_objs_ref]
                    for target_obj_name_item in applies_to_objs_ref:
                        if (
                            isinstance(target_obj_name_item, str)
                            and target_obj_name_item
                        ):
                            add_relationship(
                                func_name, target_obj_name_item, "applies to"
                            )

                computes_items_ref = function_data.get("computes")
                if computes_items_ref:
                    if not isinstance(computes_items_ref, list):
                        computes_items_ref = [computes_items_ref]
                    for computed_item_name_item in computes_items_ref:
                        if (
                            isinstance(computed_item_name_item, str)
                            and computed_item_name_item
                        ):
                            add_relationship(
                                func_name, computed_item_name_item, "computes"
                            )

                required_measurements_ref = function_data.get(
                    "functionRequiresMeasurements"
                )
                if required_measurements_ref:
                    if not isinstance(required_measurements_ref, list):
                        required_measurements_ref = [required_measurements_ref]
                    for req_measurement_name_item in required_measurements_ref:
                        if (
                            isinstance(req_measurement_name_item, str)
                            and req_measurement_name_item
                        ):
                            add_relationship(
                                req_measurement_name_item, func_name, "input for"
                            )
        ...
        if "components_sub_entities" in data:
            for component_data in data.get("components_sub_entities", []):
                if not isinstance(
                    component_data, dict
                ) or not component_data.get("name"):
                    continue
                ...
                if isinstance(parent_ref_str, str) and parent_ref_str:
                    match = re.match(
                        r"(?:subset_of_|component_of_|related_to_)?(.*)",
                        parent_ref_str,
                    )
                    if match:
                        inferred_parent_name = match.group(1)

                if inferred_parent_name:
                    rel_label = component_data.get("type", "relates to")
                    if rel_label == "derived_subset":
                        rel_label = "is subset of"
                    elif rel_label == "computed":
                        rel_label = "derived from"
        ...
        if "relationships" in data:
            for rel_data in data.get("relationships", []):
                ...
                if (
                    source_original_name and target_original_name
                ):  # Ensure both are not None
                    add_relationship(
                        source_original_name,
                        target_original_name,
                        rel_label,
                        is_dashed=is_dashed_link,
                    )
                    if (
                        isinstance(computation_func, str)
                        and computation_func
                        and target_original_name
                    ):
                        add_relationship(
                            computation_func,
                            target_original_name,
    ...
    try:
        ...
        if not isinstance(data, dict):
            logging.error(
                "Parsed YAML is not a dictionary. Cannot generate graph."
            )
            return None
        ...
        if hasattr(ye, "problem_mark") and ye.problem_mark:
            logging.error(
                f"  Problem at line {ye.problem_mark.line + 1}, column {ye.problem_mark.column + 1}"
            )
        return None
        ...
        )  # This will print the full traceback for any error
        return None
...
def generate_plan_graph(yaml_string):
    ...
    def generate_mermaid(data, parent=None, depth=0):
        ...
        for key, value in items:
            ...
            content_node_id = (
                f"{node_id}_content"  # Always create a content node ID
            )
                        ...
                        if isinstance(item, dict):
                            ...
                            for sub_key, sub_value in sub_items:
                                ...
                                if isinstance(sub_value, list):
                                    ...
                                    value_id = sanitize_id(
                                        f"{sub_node_id}_value"
                                    )
                                    mermaid_code.extend(
                                        [
