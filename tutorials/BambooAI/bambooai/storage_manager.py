...
import tutorials.BambooAI.bambooai.storage_manager as tbbastma
"""
...
class Chain:
    chain_id: str
    timestamp: float
    messages: Dict[str, List[Dict]] = field(
        default_factory=lambda: {
            "pre_eval_messages": [],
    ...
    def restore_interaction(self, thread_id: str, chain_id: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "select_analyst_messages": message_data.get(
                    "select_analyst_messages", []
                ),
                "eval_messages": message_data.get("eval_messages", []),
                "df_inspector_messages": message_data.get(
                    "df_inspector_messages", []
                ),
                "code_messages": message_data.get("code_messages", []),
                "plan_review_messages": message_data.get(
                    "plan_review_messages", []
                ),
                "insight_messages": message_data.get("insight_messages", []),
                "code_exec_results": chain_data["tools"]["code_exec"].get(
                    "code_exec_results", ""
                ),
                "executed_code": chain_data["tools"]["code_exec"].get(
                    "executed_code", ""
                ),
