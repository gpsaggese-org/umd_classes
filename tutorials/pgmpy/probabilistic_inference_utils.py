...
def cell3_2_create_cpd_widget(*, disease_prior: float = 0.05) -> ipywidgets.VBox:
    ...
    def _update_display(change: Optional[Dict] = None) -> None:
        ...
        :param change: Change event from widget
            - Default: `None`
        """
        with output:
...
def cell5_3_create_evidence_explorer() -> ipywidgets.VBox:
    ...
    def _update_plot(change: Optional[Dict] = None) -> None:
        ...
        with output:
            ...
            # Plot posterior.
            bars = axes[1].bar(
                ["Absent", "Present"], posterior_vals, color=colors
            )
            axes[1].set_ylabel("Probability")
            axes[1].set_title(
                "Posterior P(Disease | Evidence)", fontweight="bold"
            )
            axes[1].set_ylim([0, 1])
            for bar in bars:
                height = bar.get_height()
...
def cell6_1_plot_algorithm_comparison(
    ...
    *, figsize: Optional[Tuple[int, int]] = None
    ...
    # Set overall title and layout.
    fig.suptitle(
        "Comparing Inference Algorithms", fontweight="bold", fontsize=14
    )
    plt.tight_layout()
...
def cell8_1_larger_network_interactive() -> ipywidgets.VBox:
    ...
    def _update_larger_net(change: Optional[Dict] = None) -> None:
        ...
        with output:
            ...
            bar_colors = [
                "#4ecdc4" if method_dropdown.value == "VE" else "lightgray",
                (
                    "#ff6b6b"
