...
For more detailed Docker documentation, see
...
Optional flags:
- `-p PORT`: Use custom port (default 8888)
- `-u`: Enable Vim keybindings
- `-v`: Verbose output
...
The following core files comprise the tutorial:

- `gCastle_utils.py` — Helper functions for data generation, algorithm execution,
  evaluation, and visualization
- `gCastle.API.ipynb` / `gCastle.API.py` — API exploration notebook (Jupytext paired)
- `gCastle.example.ipynb` / `gCastle.example.py` — Complete example application
  (Jupytext paired)
