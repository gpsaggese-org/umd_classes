...
import nest_asyncio
nest_asyncio.apply()
import pydanticai_example_utils as example_utils
...
chunks = example_utils.chunk_docs(docs, DocChunk, max_chars=700, dim=_DIM)
...
search_chunks = partial(
    example_utils.search_chunks,
    doc_match_cls=DocMatch,
    dim=_DIM,
)

preview = search_chunks(chunks, "How do I download invoices?", top_k=3)
...
enforce_sources = partial(
    example_utils.enforce_sources,
    model_retry_cls=ModelRetry,
)
agent.output_validator(enforce_sources)
...
out = await example_utils.ask(agent, "How do I download invoices?", deps)
out
...
await example_utils.stream_demo(stream_agent)
...
in_scope = partial(example_utils.in_scope, in_scope_terms=IN_SCOPE_TERMS)

guarded = await example_utils.run_guarded(
    agent,
...
docs = example_utils.load_docs(DOCS_DIR)  # must return list[dict] with doc_id/title/text
chunks = example_utils.chunk_docs(docs, DocChunk, max_chars=700, dim=_DIM)
...
personalized = await example_utils.ask(
    agent,
