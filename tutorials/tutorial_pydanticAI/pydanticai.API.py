...
import nest_asyncio
import pydanticai_API_utils as api_utils
nest_asyncio.apply()
...
print("OPENAI_API_KEY:", api_utils.mask(os.getenv("OPENAI_API_KEY")))
...
agent = Agent("openai:gpt-4o-mini", deps_type=Config, tools=[api_utils.company_name])
...
asyncio.run(api_utils.run_agent(agent))
...
validator_agent.output_validator(
    partial(api_utils.validate_sources, model_retry_cls=ModelRetry)
)


# %%
try:
    api_utils.validate_sources(
        AnswerWithSources(
...
asyncio.run(api_utils.run_validator_example(validator_agent))
