...
def _jupyter_labextension_paths():
    return [{"src": "labextension", "dest": "context-chat"}]
...
def _load_jupyter_server_extension(server_app):
    ...
    logger.info(
        f"Context Chat server extension loaded. Endpoint: {route_pattern}"
    )
    logger.info(
        "LangChain integration: Set OPENAI_API_KEY environment variable to enable AI responses"
    )
