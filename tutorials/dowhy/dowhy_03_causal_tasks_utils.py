...
import tutorials.dowhy.dowhy_03_causal_tasks_utils as tdd0ctaut
"""
...
def cell1_1_plot_potential_outcomes() -> None:
    ...
    ax1.axhline(
        control_outcome.mean(), color="blue", linestyle="--", linewidth=2
    )
    ax1.text(
        0.5,
        max(outcome) - 5,
...
def cell1_2_healthcare_dataset(
    *,
    ...
    data["Medication"] = (np.random.random(n_samples) < treatment_prob).astype(
        int
    )
...
def cell1_3_education_earnings_dataset(
    *,
    ...
    data["Education"] = (np.random.random(n_samples) < education_prob).astype(
        int
    )
...
def cell1_3_interactive_iv_strength(df: pd.DataFrame) -> None:
    ...
    def _update(_change: Any = None) -> None:
        with output:
            ...
            distance_noisy = df["Distance"].values + np.random.normal(
                0, noise_level, len(df)
            )
            df_noisy = df.copy()
    ...
    *,
    ...
    for unit in range(n_units):
        ...
        for period in range(n_periods):
            ...
            outcome = (
                50 + 3 * period + 15 * treatment_active + np.random.normal(0, 2)
            )
            data.append(
                {
                    "Unit": unit,
...
def cell1_4_plot_did_trends(df: pd.DataFrame) -> None:
    ...
    ax.axvline(
        0.5, color="gray", linestyle="--", alpha=0.5, label="Policy intervention"
    )
    # Add parallel trends illustration.
    if len(periods) >= 2:
        pre_trend = control_means.iloc[1] - control_means.iloc[0]
        counterfactual_post = treated_means.iloc[1] + (
            pre_trend if len(periods) > 2 else 0
        )
...
def cell1_5_estimate_cate(
    ...
    *,
    ...
    stratum_labels = pd.qcut(
        df[by_var], q=n_groups, labels=False, duplicates="drop"
    )
    cate_estimates = []
...
def cell1_5_plot_cate_heterogeneity(cate_df: pd.DataFrame) -> None:
    ...
    colors = [
        "green" if c > 0.15 else "orange" if c > 0.05 else "red" for c in cates
    ]
    ax.bar(groups, cates, color=colors, alpha=0.7)
    ax.axhline(
        0.2, color="black", linestyle="--", linewidth=2, label="True ATE ≈ 0.2"
    )
    ax.set_ylabel("Conditional ATE")
    ax.set_xlabel("Disease Severity Group")
...
def cell1_5_interactive_patient_profile(df: pd.DataFrame) -> None:
    ...
    def _update(_change: Any = None) -> None:
        with output:
            ...
            window = 15
            df_local = df[
                (df["Severity"] >= severity - window)
                & (df["Severity"] <= severity + window)
            ]
            if len(df_local) > 0:
                treated = df_local[df_local["Medication"] == 1][
                    "Recovery"
                ].mean()
                control = df_local[df_local["Medication"] == 0][
                    "Recovery"
                ].mean()
            ...
            fig, ax = plt.subplots(figsize=_SINGLE_PANEL_FIGSIZE)
            ax.scatter(
                df["Severity"], df["Recovery"], alpha=0.3, label="All patients"
            )
            if len(df_local) > 0:
                ax.scatter(
                    df_local["Severity"],
...
def cell1_6_synthetic_scm_dataset(
    *,
    ...
    data["Y"] = (
        0.3 * data["X"] + 0.4 * data["M"] + np.random.normal(0, 0.5, n_samples)
    )
    # Define DAG.
    G = nx.DiGraph()
...
def _generate_from_fitted_scm(
    ...
    for node in nx.topological_sort(G):
        if node in interventions:
            ...
            X = np.column_stack(
                [np.ones(n_samples)] + [data[p] for p in parents]
            )
...
def cell1_6_estimate_effect_via_gcm(
    ...
    *,
    ...
    samples_0 = _generate_from_fitted_scm(
        G, fitted_params, n_samples_do, {treatment: 0}
    )
    samples_1 = _generate_from_fitted_scm(
        G, fitted_params, n_samples_do, {treatment: 1}
    )
...
def cell2_1_mediation_dataset(
    *,
    ...
    # Mediator: experience.
    data["Experience"] = (
        5 + 3 * data["Education"] + np.random.normal(0, 2, n_samples)
    )
    # Outcome: earnings.
    data["Earnings"] = (
        30000
...
def cell2_1_estimate_mediation(df: pd.DataFrame) -> Dict[str, float]:
    ...
    X_de = np.column_stack(
        [np.ones(len(df)), df["Education"].values, df["Experience"].values]
    )
...
def cell2_1_plot_mediation_pathways(
    ...
    labels = [
        f"Direct ({mediation_results['nde']:.0f}$)",
        f"Indirect ({mediation_results['nie']:.0f}$)",
    ]
...
def cell2_2_supply_chain_dataset(
    *,
    ...
    data["Inventory"] = (
        50 + 0.6 * data["Demand"] + np.random.normal(0, 10, n_samples)
    )
    ...
    data["Price"] = (
        50 + 0.8 * data["ProdCost"] + np.random.normal(0, 5, n_samples)
    )
...
def cell2_2_plot_weighted_dag(
    ...
    if strength_max > strength_min:
        normalized = (strength_values - strength_min) / (
...
def cell3_1_interactive_anomaly_dashboard(
    ...
    def _display() -> None:
        with output:
            ...
            deviations = [
                anomaly.get(var, baseline[var]) - baseline[var] for var in inputs
            ]
            ...
            contributions_norm = [
                c / sum(contributions) * 100 if sum(contributions) > 0 else 0
            ...
            ax2.pie(
                ...
                labels=[
                    f"{inp} ({c:.1f}%)"
                    for inp, c in zip(inputs, contributions_norm)
                ],
                startangle=90,
            )
...
def cell3_2_compute_shift_attribution(
    ...
    outcome_change = (
        df_after["Satisfaction"].mean() - df_before["Satisfaction"].mean()
    )
    feature_changes = []
...
def cell3_2_plot_shift_attribution(shift_df: pd.DataFrame) -> None:
    ...
    ax2.pie(
        contributions_norm,
        labels=[f"{f} ({c:.1f}%)" for f, c in zip(features, contributions_norm)],
        startangle=90,
    )
...
def cell3_3_compute_causal_relevance(
    ...
    return pd.DataFrame(causal_effects).sort_values(
        "Causal_Effect", ascending=False
    )
...
def cell3_3_compute_statistical_relevance(
    ...
    return pd.DataFrame(stat_relevance).sort_values(
...
def cell3_3_plot_causal_vs_statistical(
    ...
    ax2.barh(
        stat_df["Feature"], stat_df["Correlation"], color="coral", alpha=0.7
    )
...
def cell4_1_plot_dose_response(dose_response_df: pd.DataFrame) -> None:
    ...
    fig, ax = plt.subplots(figsize=_SINGLE_PANEL_FIGSIZE)
    ax.plot(
        dose_response_df["Spend"],
        dose_response_df["Revenue"],
        marker="o",
        linewidth=2,
    )
...
def cell4_1_interactive_intervention(
    ...
    def _update(_change: Any = None) -> None:
        with output:
            ...
            samples = _generate_from_fitted_scm(
                G, fitted_params, 200, {"MarketingSpend": spend}
            )
            mean_revenue = samples["Revenue"].mean()
            fig, ax = plt.subplots(figsize=_SINGLE_PANEL_FIGSIZE)
            ax.hist(
            ...
            ax.set_title(
                f"Revenue Distribution given Marketing Spend = ${spend:.0f}"
            )
            ax.legend()
            plt.tight_layout()
            plt.show()
    ...
    *,
    ...
    result = {
        ...
        "individual_effect": float(
            individual_cf.get("Outcome", 0) - individual.get("Outcome", 0)
        ),
    }
    return result
...
def cell4_3_customer_support_dataset(
    *,
    ...
    data["PremiumSupport"] = (np.random.random(n_samples) < support_prob).astype(
        int
    )
...
def cell4_3_plot_policy_comparison(df: pd.DataFrame) -> None:
    ...
    ax2.bar(
        ["Optimal Allocation"], [optimal_satisfaction], color="green", alpha=0.7
    )
    ...
    def _update(_change: Any = None) -> None:
        with output:
            ...
            if len(df_treated) > 0:
                ...
                policy_satisfaction = df[df["PremiumSupport"] == 1][
                    "Satisfaction"
                ].mean()
            fig, ax = plt.subplots(figsize=_SINGLE_PANEL_FIGSIZE)
            ax.barh(
...
def cell5_1_plot_ood_comparison(
    ...
    ax.scatter(
        df_test["X"], df_test["Y"], alpha=0.5, label="OOD test data", color="red"
    )
...
def cell5_2_two_population_dataset() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    pop_a = pd.DataFrame(
        {"Age": a_age, "Treatment": a_treatment, "Outcome": a_outcome}
    )
    ...
    pop_b = pd.DataFrame(
        {"Age": b_age, "Treatment": b_treatment, "Outcome": b_outcome}
    )
    return pop_a, pop_b
...
def cell5_2_interactive_population_comparison(
    ...
    def _display() -> None:
        with output:
            ...
            axes[0].hist(
                df_target["Age"], bins=20, alpha=0.5, label="Target", color="red"
            )
            ...
            axes[1].bar(
                ["Source", "Target"],
                [source_treat_rate, target_treat_rate],
                color=["blue", "red"],
                alpha=0.7,
            )
        ...
        with output:
            ...
            if goal == "Estimate effects":
                if confounders == "No":
                    ...
                    recommendation = (
                        "Backdoor adjustment (regression, matching, IPW)"
                    )
                else:
                    recommendation = (
                ...
                recommendation = (
                    "Mediation analysis, arrow strength, feature relevance"
                )
            elif goal == "Root cause analysis":
                recommendation = (
                    "Anomaly attribution, distributional decomposition"
                )
            else:
...
def cell6_2_ecommerce_dataset(
    *,
    ...
    # Marketing spend (treatment).
    data["MarketingBudget"] = data["BaseBudget"] * np.random.uniform(
        0.01, 0.1, n_samples
    )
    ...
    data["Revenue"] = data["Traffic"] * data[
        "Conversion"
    ] * 100 + np.random.normal(0, 5000, n_samples)
    # Define DAG.
    G = nx.DiGraph()
    G.add_edges_from(
        [
...
def cell6_2_run_full_analysis(
    ...
    # GCM-based estimate.
    gcm_result = cell1_6_estimate_effect_via_gcm(
        df, G, treatment="MarketingBudget", outcome="Revenue"
    )
