...
class Test_fit_dml_model(hunitest.TestCase):
    """
    Test the fit_dml_model function.
    """

    def _prepare_df(self):
        """
        Build a small simulated DataFrame for causal inference tests.
        """
        agents = racaosimscsu.create_population(n_agents=40, seed=42)
        racaosimscsu.run_simulation(agents, n_periods=20, seed=42)
        return racaosimscsu.get_results_dataframe(agents)

    def test1(self) -> None:
        """
        Test fit_dml_model returns four values of the expected types.
        """
        # Prepare inputs.
        df = self._prepare_df()
        # Run test.
        dml_model, ate_estimates, ate_mean, ate_std = racaosimscsu.fit_dml_model(
            df, n_estimators=10, max_depth=5, random_state=42
        )
        # Check outputs.
        self.assertIsNotNone(dml_model)
        self.assertEqual(len(ate_estimates), len(df))
        self.assertIsInstance(ate_mean, float)
        self.assertIsInstance(ate_std, float)

    def test2(self) -> None:
        """
        Test fit_dml_model is deterministic under a fixed random_state.
        """
        # Prepare inputs.
        df = self._prepare_df()
        # Run test.
        _, _, ate_mean1, _ = racaosimscsu.fit_dml_model(
            df, n_estimators=10, max_depth=5, random_state=42
        )
        _, _, ate_mean2, _ = racaosimscsu.fit_dml_model(
            df, n_estimators=10, max_depth=5, random_state=42
        )
        # Check outputs.
        self.assertAlmostEqual(ate_mean1, ate_mean2, places=6)


# #############################################################################
# Test_fit_causal_forest
# #############################################################################


class Test_fit_causal_forest(hunitest.TestCase):
    """
    Test the fit_causal_forest function.
    """
    ...
    def test1(self) -> None:
        """
        Test fit_causal_forest returns a model and per-agent CATE estimates.
        """
        # Prepare inputs.
        df = self._prepare_df()
        # Run test.
        cf_model, cate_estimates = racaosimscsu.fit_causal_forest(
            df, n_estimators=10, max_depth=5, random_state=42
        )
        # Check outputs.
        self.assertIsNotNone(cf_model)
        self.assertEqual(len(cate_estimates), len(df))


# #############################################################################
# Test_compare_allocation_policies
# #############################################################################


class Test_compare_allocation_policies(hunitest.TestCase):
    """
    Test the compare_allocation_policies function.
    """

    def _prepare_df_with_cate(self):
        """
        Build a simulated DataFrame with a 'cate' column.
        """
        agents = racaosimscsu.create_population(n_agents=60, seed=42)
        racaosimscsu.run_simulation(agents, n_periods=20, seed=42)
        df = racaosimscsu.get_results_dataframe(agents)
        # Add synthetic CATE estimates so the CATE-optimal policy can run.
        df["cate"] = np.linspace(0.01, 0.5, len(df))
        return df

    def test1(self) -> None:
        """
        Test policy comparison returns five rows and expected columns.
        """
        # Prepare inputs.
        df = self._prepare_df_with_cate()
        # Run test.
        comparison = racaosimscsu.compare_allocation_policies(df)
        # Check outputs.
        ...
        self.assertEqual(set(comparison["Policy"]), expected_policies)
        for col in [
            "Total Capital",
            "Gini Coefficient",
            "Top 10% Share",
    ...
    def test2(self) -> None:
        """
        Test Gini coefficients from policy comparison stay in [0, 1].
        """
        # Prepare inputs.
        df = self._prepare_df_with_cate()
        # Run test.
        comparison = racaosimscsu.compare_allocation_policies(df)
        # Check outputs.
        for gini in comparison["Gini Coefficient"]:
            self.assertGreaterEqual(gini, 0.0)
            self.assertLessEqual(gini, 1.0)
