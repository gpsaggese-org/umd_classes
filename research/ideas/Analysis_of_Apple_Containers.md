...
- **Apple Containers** is a macOS-native, open-source container runtime released
  by Apple at WWDC 2025
...
Apple Silicon Mac, quantifying startup latency, memory footprint, CPU usage, and
inference throughput on a real NLP dataset.
...
- Verify prediction accuracy against the ground-truth labels (expect > 90% F1 on
...
- **Larger model**: swap in `bert-base-uncased` (fine-tuned on SST-2) to amplify
  memory and latency differences between runtimes
