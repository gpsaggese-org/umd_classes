...
\graphicspath{{papers/gp_saggese_quant_research_cv/figures}}
...
% ==============================================================================
  ...
  I am a quantitative researcher specializing in applying machine learning (ML)
  and artificial intelligence (AI) to financial problems, ranging from
  alpha-strategies to portfolio construction.

  \textbf{As a quant}, my experience spans developing, trading, and evaluating
  over 100 alpha signals across:
  \begin{itemize}
    \item \textit{asset classes} (US equities, futures, crypto)
    \item \textit{trading frequencies} (from minutes to days)
    \item utilizing both \textit{market microstructure} and \textit{alternative
      data sources}
  \end{itemize}

  \textbf{As a technologist}, I have developed proprietary research
  infrastructure including:
  \begin{itemize}
    \item alpha signal \textit{backtesting frameworks}
    \item \textit{portfolio optimization} systems
    \item \textit{market impact models}
  \end{itemize}

  \textbf{In leadership roles}, I interviewed 50+ quantitative analysts, led
  research teams, and conducted thorough due diligence on alternative datasets.

  \textbf{As a lecturer in AI / ML}, I teach graduate-level courses including
  ``DATA605: Big Data Systems'' and ``MSML610: Advanced Machine Learning'' at
  the University of Maryland, College Park (ranked 9th among public US
  universities), reaching over 800 students since 2020.

  \textbf{As an academic researcher} at the University of Maryland I've focused
  on \textit{Bayesian statistics} and \textit{causal AI}, specifically
  developing methods to:

  \begin{itemize}
    \item Distinguish \textit{causal relationships} from spurious correlations
      in financial time series, enabling models that capture true economic
      mechanisms rather than data-mining artifacts

    \item Quantify uncertainty in model predictions using \textit{Bayesian
      hierarchical models}, providing principled confidence intervals for
      trading decisions

    \item Build \textit{knowledge graphs} that encode economic relationships
      across asset classes, sectors, and macroeconomic factors

    \item Apply \textit{temporal machine learning} to adapt to regime changes
      while maintaining interpretability of learned patterns
  \end{itemize}

  I have successfully applied these methodologies to financial prediction,
  predictive maintenance, demand forecasting, and energy trading, and
  demonstrated that they consistently improve both accuracy and robustness
  over traditional approaches.

  \textbf{Quantifiable Impact}:

  \begin{itemize}
    \item Contributed \textit{25\% of profits} to hedge funds managing
      \$1.6B in AUM across global futures and US equities

    \item Generated \textit{\$90M in profits} with a Sharpe ratio of
      approximately \textit{3} over 18 months
      achieving \textit{56\% returns} at 2x volatility target

    \item Successfully deployed \textit{over 100 alpha signals} in
      production across US equities, global futures, and ETFs, achieving
      \textit{Sharpe ratios ranging from 1.5 to 5.0} across different
      strategies

    \item Built and led research teams totaling \textit{15+ quantitative
      researchers} across two offices (Berkeley, CA and Moscow, Russia),
      integrating alternative data sources (e.g., news sentiment, social media,
      macroeconomic indicators, and SEC filings) into quantitative strategies

    \item Built \textit{two complete trading systems} from data ingestion
      through portfolio construction
  \end{itemize}

  My research philosophy centers on using modern AI and machine learning to
  complement, not replace, traditional quantitative methods, maintaining a
  high-throughput research process where models are discovered, validated, and
  deployed at a predictable rate while accounting for their limited lifespan in
  competitive markets.

  \begin{itemize}
    \item \href{https://gpsaggese.github.io/}{Complete CV and additional materials}
    ...
    \item \href{https://gpsaggese.github.io/06_research/}{AI Research}
  \end{itemize}
...
% ==============================================================================
  ...
  My research in causal AI and bayesian statistics at the University of
  Maryland, and applied work at Causify.AI, Teza, Engineers' Gate, informs my
  ...
  \begin{itemize}
    \item \textbf{Causal Inference}: Moving beyond correlation to understand
      \textit{causal relationships} in financial markets, building models
      that capture underlying economic mechanisms rather than spurious
      patterns

    \item \textbf{Bayesian Methods}: Incorporating \textit{prior knowledge}
      and \textit{uncertainty quantification} into model development ---
      enabling principled handling of parameter uncertainty and model
      selection

    \item \textbf{Decision-Making Under Uncertainty}: Combining
      \textit{probabilistic reasoning} with temporal machine learning to
      build adaptive models that adjust to changing market regimes and
      account for epistemic uncertainty
  \end{itemize}

  This foundation shapes my philosophy on alpha generation, where
  \textit{interpretability}, \textit{economic reasoning}, and \textit{rigorous
  statistical validation} are paramount.
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    \item Models must be \textit{interpretable} and grounded in \textit{economic
      theory}

    \item \textit{State-of-the-art machine learning} should complement
      traditional statistical methods --- not replace them

    \item \textit{High-throughput research processes} that account for the
      limited lifespan of alpha signals in competitive markets, ensuring models
      are discovered and deployed in production at a predictable, steady rate

    \item Maximizing the impact of \textit{alternative data} means
      investigating signals:

      \begin{itemize}
        \item At different time scales, from minutes to days

        \item Across multiple asset classes (futures, equities, ETFs, FX,
          etc.)
    ...
    \item Designing and implementing sophisticated, proprietary data
      pipelines --- in addition to expertise in quant methodologies --- is
      essential

    \item Strong engineering practices combined with rigorous research and
      development (R\&D) accelerate results
  \end{itemize}
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    ...
    \item \textbf{Iterative Improvement}: Progressively refine optimization
      to address signal peculiarities (e.g., sparsity, differing time scales)

    \item \textbf{Leverage Existing Infrastructure}: Utilize available
      components for:

      \begin{itemize}
        \item \textit{Mean-variance optimization} with standard constraints

        \item \textit{Covariance estimation}

        \item \textit{Transaction cost models}
  ...
  \begin{itemize}
    \item \textit{Market neutrality} (primary concern)

    \item \textit{Low sector exposure}

    \item \textit{Factor exposure controls} (verified ex-post)

    \item Integration with commercial factor models for optimization (e.g.,
      Barra, Axioma)
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item \textit{Mixer} blends forecasts from all models
  ...
  \begin{itemize}
    ...
    \item Incorporate actual models into backtesting and optimization systems

    \item Account for costs and optimize for target trading backend

    \item API-based target portfolio specification for each period
  \end{itemize}
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    \item \textit{Large coverage} of the trading universe

    \item \textit{Frequent updates}

    \item \textit{Usability across asset classes} (e.g., equities and
  ...
  \begin{itemize}
    \item News (headlines and full-text via NLP)

    \item Social sentiment (blogs, Twitter, StockTwits)

    \item Anonymized spending reports, credit card transactions, payroll
      data

    \item Macroeconomic data (nowcasting inflation, industrial production,
      housing starts)

    \item Business reporting and disclosures

    \item Smartphone geolocation data (e.g., retail traffic)

    \item Supply chain information (trucking, rail freight)

    \item Satellite data (agriculture, ship tracking)

    \item Online retail prices (inflation tracking)

    \item Weather data
  \end{itemize}

  \textbf{Models Developed and Deployed (Sharpe Ratios)}:

  \begin{itemize}
    \item \textit{News sentiment} --- Bloomberg, Reuters, Ravenpack
      ($\approx 1.5-2.0$)

    \item \textit{Social sentiment} --- Twitter and StockTwits
      ($\approx 1.0-2.0$)

    \item \textit{SEC insider trading} signals ($> 0.5$)

    \item \textit{SEC fundamental analysis} with Compustat ($> 2.0$)

    \item \textit{Value strategies} combining Compustat and news
      ($> 2.0$)

    \item \textit{Oil / natural gas} using news and 15-min futures
      ($3-5$)

    \item \textit{Macroeconomic signals} from supply chain datasets
      ($\approx 1-1.5$)

    \item \textit{Spot FX} using purchasing power parity ($> 2-3$)

    \item \textit{STIR / bond futures arbitrage} with treasury auction data
      ($\approx 1.5-2$)

    \item \textit{Microstructure models} using order book data ($> 3.0$)
  \end{itemize}
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    \item \textit{US equities} --- market neutral, sector neutral, directional
      strategies

    \item \textit{Global futures} --- outrights, calendar spreads across
      financials, energy, interest rates, currencies, commodities, and
      agriculture

    \item \textit{US ETFs} for factor and sector prediction strategies
  \end{itemize}
  ...
  \begin{itemize}
    \item \textit{Mid-frequency strategies} ranging from minutes to days

    \item Optimal frequency depends on the alpha model, the specific
      alternative data set, and transaction cost estimates
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Some data sets (news, social sentiment, blogs) support higher
      frequencies, from 15-minute intervals down to every minute

    \item Other data sets drive event-driven trading (earnings, M\&A,
      executive departures, credit rating changes) on time scales of
      hours, days, or weeks

    \item Some models combine market microstructure data with fast
      alternative data (e.g., news) to trade on minutely or hourly
      time scales
  \end{itemize}
...
% ==============================================================================
  ...
  \begin{enumerate}
    ...
    \item \textbf{Data Isolation}: Separate holdout data \textit{before}
    ...
    \item \textbf{Independent Review}: Audit analysis and implementation
      with fresh eyes

    \item \textbf{Out-of-Sample (OOS) Validation}: Open the OOS box only when
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    \item Accelerate how research in finance is done --- not change the basics
      of quant research
  ...
  \begin{itemize}
    \item \textbf{Simple First}: \textit{Simple models should always be preferred}
      over complex ones

    \item \textbf{Balance Complexity}: \textit{Strike a balance between model complexity}
...
% ==============================================================================
  ...
  \begin{itemize}
    ...
    \item Research pipeline (e.g., feature computation, hypothesis testing,
      cross-validation)
  ...
  \begin{itemize}
    \item Cloud infrastructure: Amazon AWS (EC2, S3, RDS) for scalable compute
      and storage

    \item Data pipelines: Real-time and batch processing for market and
      alternative data

    \item Version control: Git for code, tools for data versioning
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    \item Joined as the second person in the alternative data group in Berkeley

    \item No existing research pipeline for ``low frequency'' trading,
      no data APIs

    \item Research and trading infrastructure was geared towards high-frequency
      trading (HFT) with a pervasive assumption that nothing was held overnight
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    \item \textit{Deployed $\approx$ 40 models} across multiple strategies:
    ...
    \item Research hit rate $\approx$ 50\% (2-3 data sets didn't
      pan out)

    \item Historical Sharpe ratio $\approx$ 4-5 for futures, before
      costs
  \end{itemize}

  \textbf{Futures Returns (Jan 2016 - Jan 2017):}
...
% ------------------------------------------------------------------------------
  ...
  \begin{itemize}
    ...
    \item 1B USD assets under management (AUM)

    \item 25\% of risk allocated to alternative data

    \item Mid-2016 to end of 2017: Sharpe ratio $\approx$ 3, \$90M profit
      and loss (PnL)

    \item 2x volatility target → returns $\approx$ 56\%
  \end{itemize}
...
% ------------------------------------------------------------------------------
  ...
  In 2018, the alternative data team released $\approx$ 10 signals from
  4-5 data sets:
...
% ------------------------------------------------------------------------------
  ...
  Figure~\ref{fig:us-equity-daily} shows the cumulative performance of daily
  US equity strategies, demonstrating consistent returns across multiple
  event-driven models with strong risk-adjusted performance.

  \begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{image8.png}
    \caption{Daily US equity strategy cumulative performance}
    \label{fig:us-equity-daily}
  \end{figure}
...
% ------------------------------------------------------------------------------
  ...
  Figures~\ref{fig:us-equity-intraday} illustrate the intraday US equity
  strategy performance, showing cumulative returns and drawdown characteristics
  for high-frequency microstructure-based models achieving Sharpe ratios
  exceeding 5.

  \begin{figure}[htbp]
    \centering
    \includegraphics[width=0.50\linewidth]{image6.png}

    \includegraphics[width=0.50\linewidth]{image7.png}

    \caption{Intraday equity strategy performance and drawdowns}
    \label{fig:us-equity-intraday}
  \end{figure}
...
% ------------------------------------------------------------------------------
  ...
  Between 2015 and 2019 I was head of data at a multi-strategy firm. My main
  duty was to produce alpha strategies for our funds. I've supervised a team of
  5 researchers in Berkeley, CA and around 10 researchers / developers in the
  Moscow, Russia office.

  There were two funds from external investors:
  ...
  Figure~\ref{fig:global-futures-performance} presents the realized performance of the global futures fund
  ...
  \begin{figure}[htbp]
    ...
    \caption{Global futures fund realized performance (2015-2019)}
    \label{fig:global-futures-performance}
  \end{figure}

  Figures~\ref{fig:us-equity-performance} and~\ref{fig:us-equity-risk} show the US equity fund performance launched in 2017,
  ...
  \begin{figure}[htbp]
    ...
    \caption{US equity fund performance (2017-2019)}
    \label{fig:us-equity-performance}
  \end{figure}
  ...
  \begin{figure}[htbp]
    ...
    \caption{US equity fund risk metrics and returns}
    \label{fig:us-equity-risk}
  \end{figure}

  Figure~\ref{fig:strategy-composition} details the composition and diversification across strategy types,
  ...
  \begin{figure}[htbp]
    ...
    \caption{Strategy composition and diversification}
    \label{fig:strategy-composition}
  \end{figure}
...
% ------------------------------------------------------------------------------
  ...
  The following example illustrates alpha HFT strategies for cryptocurrency markets.
  ...
  \begin{itemize}
    ...
    \item \emph{Performance}: Figure~\ref{fig:crypto-statarb} displays the cumulative returns of
      ...
      \begin{figure}[htbp]
        ...
        \caption{Cryptocurrency statistical arbitrage cumulative returns}
        \label{fig:crypto-statarb}
      \end{figure}
      ...
      \begin{itemize}
        ...
        \item \emph{Sharpe ratio} (measure of risk-adjusted returns): $\approx$ 7
    ...
    \item \emph{Holding period}: around 30 mins
      ...
      \begin{itemize}
        ...
        \item The entire book can be liquidated within 15 minutes
      \end{itemize}

    \item \emph{Brief description}: Every 5 to 15 minutes, a proprietary
      statistical arbitrage model computes the optimal portfolio via Bayesian
      mean-variance optimization, given current holdings, ideal holdings as
      predicted by the model, cost to execute,
