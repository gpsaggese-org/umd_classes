...
% TODO(gp): Pin down one concrete instantiation of the task unit $\calU$
% (e.g., a benchmark-normalized task-equivalent computed against a reference
% model) instead of leaving it fully abstract. Every worked example in the
% paper (100 tasks, 10,000 tasks, etc.) already treats $\calU$ as settled, so
% the abstraction is undermining the concreteness of the notation it is
% supposed to support. Show that the chosen definition satisfies addition and
% total order, and discuss its cross-provider comparability.
