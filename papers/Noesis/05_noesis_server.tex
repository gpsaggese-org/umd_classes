...
% TODO(ai_gp): Add a sequence diagram of a request's path through
% NoesisServer: caller -> API gateway -> provider, with the logged tuple
% (prompt, response, provider, model, latency, cost, contract-or-bottom)
...
% TODO(ai_gp): Add a figure illustrating the hypothesis test of
% Equation~\eqref{eq:reliability_lower_bound}: the point estimate
% $\hat r(\kappa, W)$, its lower confidence bound $\hat r_{\text{lower}}$,
% and the threshold $\rminattr(\kappa)$ on a single number line, showing
% when a shortfall is flagged versus attributed to noise.
...
% TODO(ai_gp): Add a flowchart of difficulty-aware routing: prompt $x$ ->
% difficulty estimator $\hat d$ -> cheapest sufficient tier $\pi(x)$
% (Equation~\eqref{eq:routing_policy}), next to the cost comparison of
% Equation~\eqref{eq:routing_saving_condition}.
...
% TODO(ai_gp): Add a figure showing answer fusion as a fan-out/fan-in: one
% prompt sent to several providers in $S(x)$, whose answers are combined
% by the aggregator $g$ (Equation~\eqref{eq:fusion_aggregator}), contrasted
% side by side with routing's single-provider path.
...
% TODO(ai_gp): Add a figure showing the training pipeline: logged
% prompt/response traffic -> agreement labels (Definition~\ref{def:agreement_label})
% feeding the difficulty estimator, and the same corpus $\calD$ feeding
% distillation of $f_\theta$, as two parallel consumers of one dataset.
