...
% TODO(ai_gp): Add a figure contrasting the two trust boundaries: a single
% operator seeing all bids/asks and deciding eligibility (centralized), vs.
% commit-reveal plus on-chain settlement with only addresses visible
% (decentralized), to make Section~\ref{sec:decentralized_motivation}'s
% concern concrete before the mechanisms below are introduced.
...
% TODO(ai_gp): Add a sequence diagram of one auction round under commit-reveal:
% commit window (bidders post $h_\beta = H(\beta \| \nu_\beta)$) followed
% by the reveal window (bidders disclose $(\beta, \nu_\beta)$), followed by
% clearing (Problem~\ref{prob:clearing}) on only the matching, revealed bids.
...
% TODO(ai_gp): Add a state diagram for a staked ask's lifecycle: posted at
% reveal -> locked while matched into a contract -> released in full or
% slashed once $\hat r_{\text{lower}}(\kappa, W)$ is reported
% (Definition~\ref{def:staked_ask}).
...
% TODO(ai_gp): Add a figure spanning the three placements for the clearing
% problem (fully on-chain, fully off-chain, verifiably off-chain) as a
% spectrum from trust-minimized/expensive to cheap/trusted, marking where
% each placement sits.
...
% TODO(ai_gp): Add a figure showing $k_{\text{red}}$ staked asks racing to
% fulfill one contract, with only the sellers meeting the contract's terms
% retaining their stake, next to a single-ask match for comparison.
