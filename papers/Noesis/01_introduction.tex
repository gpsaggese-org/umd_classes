...
Table~\ref{tab:components} shaded in teal: the demand and supply sides
submit bids and asks to \NoesisMarket{}'s matching engine, which clears
them into contracts dispatched to \NoesisServer{}'s API gateway. The
...
capability reference, then reports fulfillment back to \NoesisMarket{}'s
reputation-and-feedback component, which gates the matching engine and
feeds its pricing-dissemination component. The gateway can also fan a
request out to several providers through the answer-fusion component
(Section~\ref{sec:fusion}) when reliability outweighs cost.
