...
% TODO(ai_gp): Add the concept of max delay (e.g., to execute workload within N
% hours for batch execution)
% TODO(ai_gp): People pay for token but they should pay for functionality.
% (virtual tokens, intelligence token)
% The fragmentation of the markets is not positive for consumers.
% TODO(ai_gp): Users don't have a way to measure quality of service (e.g,
% model nerfing, low availability)
% TODO(ai_gp): Add cache
...
% TODO(ai_gp): Use a bullet list for the components.
...
Table~\ref{tab:components} shaded in teal: the demand and supply sides submit
bids and asks to \NoesisMarket{}'s matching engine, which clears them into
contracts dispatched to \NoesisServer{}'s API gateway. The gateway proxies
matched requests to the underlying providers through per-provider APIs, and its
metering logic checks the responses against the contract terms, using external
intelligence-measure providers (e.g.,
\footnote{\url{https://artificialanalysis.ai}}) as a capability reference, then
reports fulfillment back to \NoesisMarket{}'s reputation-and-feedback
component, which gates the matching engine and feeds its pricing-dissemination
component. The gateway can also fan a request out to several providers through
the answer-fusion component (Section~\ref{sec:fusion}) when reliability
outweighs cost.

% TODO(ai_gp): Reorganize it visually so that the figure can be bigger and more 
% readable. Add a color legend for Noesis, Providers / supply
...
% caption=The \Noesis{} architecture, with its five pluggable components shaded in teal.
% rendered_images:end
% render_images:begin
\begin{figure}[H]
...
% TODO(ai_gp): Component, What it does, Section, This paper implementation, Alternative implementations
\begin{samepage}
  \begin{table}[H]
    \centering
    \footnotesize
...
% TODO(ai_gp): Answer fusion and difficulty-aware routing are part of the same
% component.
...
% TODO(ai_gp): Remove textbf from the table
\begin{samepage}
  \begin{table}[H]
    \centering
...
% TODO(ai_gp): Split in bullet points
