...
% TODO(gp): Add a cluster (and matching related-work citation) for
% agent-to-agent payment / commerce protocols, e.g., Coinbase's x402 and
% Google's Agent Payments Protocol (AP2), which target the same buyer
% Noesis targets, autonomous agents that need to pay for a service
% programmatically. These are the most directly comparable, currently-live
% infrastructure and are missing from every cluster below. Positioning
% Noesis against them sharpens the "autonomous agents" framing and may
% reveal a natural integration point, Noesis as the discovery/settlement
% layer running under those payment rails rather than a competitor to them.
...
% \draw[->, >=stealth, thick, axiscolor] (0,0) -- (9.6,0) node[right, font=\scriptsize, align=left] {Auction-based\\price discovery};
% \draw[->, >=stealth, thick, axiscolor] (0,0) -- (0,7.4) node[above, font=\scriptsize, align=center] {Bundled, metered\\quality};
% \draw[dashed, dividercolor] (4.6,-0.3) -- (4.6,7.4);
% \draw[dashed, dividercolor] (-0.3,3.6) -- (9.6,3.6);
% \node[font=\scriptsize] at (2.3,-0.5) {No};
% \node[font=\scriptsize] at (7.0,-0.5) {Yes};
% \node[font=\scriptsize] at (-0.6,1.8) {No};
% \node[font=\scriptsize] at (-0.6,5.5) {Yes};
% \filldraw[ptcolor] (0.9,0.9) circle (2.4pt);
% \node[anchor=west, align=left, font=\scriptsize] at (1.3,0.9) {Learned routers};
% \filldraw[ptcolor] (0.9,2.4) circle (2.4pt);
% \node[anchor=west, align=left, font=\scriptsize] at (1.3,2.4) {Posted-price gateways};
% \filldraw[ptcolor] (0.9,3.3) circle (2.4pt);
% \node[anchor=west, align=left, font=\scriptsize] at (1.3,3.3) {Serverless inference providers};
% \filldraw[ptcolor] (0.9,4.9) circle (2.4pt);
% \node[anchor=west, align=left, font=\scriptsize] at (1.3,4.9) {Token-incentivized\\inference networks};
% \filldraw[ptcolor] (5.2,0.9) circle (2.4pt);
% \node[anchor=west, align=left, font=\scriptsize] at (5.6,0.9) {Reverse-auction \& spot compute};
% \filldraw[ptcolor] (5.2,2.1) circle (2.4pt);
% \node[anchor=west, align=left, font=\scriptsize] at (5.6,2.1) {Compute exchanges \& derivatives};
% \filldraw[noesiscolor] (8.8,6.6) circle (3pt);
% \node[anchor=west, align=left, font=\scriptsize\bfseries, text=noesiscolor] at (8.3,7.05) {Noesis};
% \end{scope}
% ```
