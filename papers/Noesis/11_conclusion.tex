...
% TODO(gp): Add empirical validation, even at minimal scale, before the
% next revision: build the auction simulator and logging proxy mentioned
% below and actually run them, e.g., simulate clearing-price convergence
% and reputation-score dynamics under a misbehaving seller, and measure
% real $ savings from difficulty-aware routing (Equation~\eqref{eq:routing_saving_condition})
% against a real gateway such as OpenRouter. This is the single biggest
% gap for the paper's impact: right now every claim is design-only, and
% the paragraph below admits as much.
