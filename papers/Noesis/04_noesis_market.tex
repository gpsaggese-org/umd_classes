...
% TODO(gp): Resolve, rather than defer, the mismatch between this
% $T$-minute periodic clearing and interactive demand (chat agents, coding
% assistants) that needs a synchronous, sub-second response. Spell out
% explicitly how an individual request inside a fulfillment window is served
% against a pre-cleared capacity contract (i.e., that $T$ governs when
% capacity and price are cleared, not the latency of any one request), and
% what happens on overage within a window or for a request that arrives
% between rounds (spot fallback). This is currently only gestured at as a
% future hybrid design in Section~\ref{sec:open_questions_market}, but it is
% load-bearing for the whole premise and should be addressed in the main
% design, not just the open-questions list.
...
% TODO(gp): Replace the citation-only treatment above with at least one
% formal result tailored to Noesis rather than a pointer to the general
% auction-theory literature. Concretely: connect the reliability lower
% bound of Equation~\eqref{eq:reliability_lower_bound} to a
% minimum-detectable-shortfall analysis, i.e., derive how large a true
% reliability shortfall a rational seller can sustain, as a function of the
% window size $|W|$ and confidence level $\delta$, before
% $\hat r_{\text{lower}}(\kappa, W) < \rminattr(\kappa)$ triggers a
% violation. This turns the disclaimed "we do not resolve this" below into
% a concrete, checkable bound on the capability-misrepresentation risk.
