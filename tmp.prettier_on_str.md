...
- Propensity score weighting is an alternative to linear regression for
...
- Extension available for continuous treatments via generalized propensity score
  (GPS)
...
- Tech companies often transition talented individual contributors to management
  roles
...
- Confounders: `tenure`, `n_of_reports`, `gender`, `role`, `department_size`,
  `department_score`, `last_engagement_score`
- Challenge: noncompliance (some assigned managers didn't attend, others
  attended without assignment)
...
- Regression with confounders: ATE = 0.2677 (adjusted estimate shows positive
  bias existed)
...
- Acts as dimensionality reduction: instead of conditioning on high-dimensional
  $X$, condition on single propensity score
...
- Freilich-Wooldridge-Newey (FWL) theorem: linear regression models
  $E[T \mid X]$ in debiasing step
- Propensity score estimation via logistic regression achieves similar
  orthogonalization
- Can use estimated propensity score $\hat{e}(X)$ in regression:
  engagement_score ~ intervention + propensity_score
- Result ($\hat{ATE} = 0.263$) remarkably similar to regression with all
  covariates
...
- Drawbacks (author's view):
  ...
  - KNN inefficient in high dimensions (though less issue when using propensity
    score alone)
  - Not recommended compared to inverse propensity weighting

### Bias Correction for Matching

- Can add bias correction using outcome models $\hat{\mu}_0(X)$ and
  $\hat{\mu}_1(X)$
...
- Treated individuals with low propensity look like untreated → high weight to
  estimate $Y_1 \mid T=0$
- Control individuals with high propensity look like treated → high weight to
  estimate $Y_0 \mid T=1$
- Balances treatment groups to appear randomly assigned

### IPW Implementation

- Estimate propensity score via logistic regression
- Calculate weights: $w_t = 1/\hat{e}(X)$ for treated,
  $w_{nt} = 1/(1-\hat{e}(X))$ for control
- Compute weighted means and take difference
- Result in management training data: $\hat{ATE} = 0.266$ (similar to
  regression)
...
- Effective sample size of pseudo-population matches original treated/control
  group sizes
...
- Same logic from Chapter 4: variables predicting $T$ but not causing $Y$ only
  add variance
- Must balance modeling bias sources vs. inducing high variance

### Trimming Strategy

- Trim propensity scores to avoid extreme weights (e.g., keep between 1% and
  99%)
...
- Positivity: every unit has positive probability of receiving each treatment
  level
...
- More covariates in propensity model → stronger unconfoundedness but weaker
  positivity
...
- Combines propensity score weighting (design-based) with outcome modeling
  (model-based)
...
- Rewrite as:
  $\hat{\mu}_t^{DR} = \frac{1}{N}\sum\frac{TY}{\hat{e}(X)} + \frac{1}{N}\sum\frac{T - \hat{e}(X)}{\hat{e}(X)}\hat{m}(X)$
- Second term converges to zero (since $T - \hat{e}(X) \approx 0$ when
  propensity model correct)
...
- Formula for normal:
  $f(t_i) = \frac{\exp(-(1/2)((t_i - \mu_i)/\sigma)^2)}{\sigma\sqrt{2\pi}}$
...
- More interpretable: upweight units with unlikely treatment combinations with
  $x$ values
...
- **IPW**: Models $P(T \mid X)$, reweights by inverse propensity, creates
  pseudo-population
