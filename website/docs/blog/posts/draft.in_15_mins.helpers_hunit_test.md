---
title: "An AI-friendly Unit Test Framework"
draft: true
authors:
    - gpsaggese
date: 2026-06-14
...
- `hunit_test` is a Python testing framework built on top of `unittest` and
  `pytest` that adds three critical capabilities:
    - **Golden file testing**: easy to compare output against a reference file
      stored in the repo or a string
  ...
  - Stores reference outputs in versioned files alongside test code at test/outcomes/
  - Enables pytest --update_outcomes — a single command to accept intentional changes
  - This is especially designed for AI-assisted development: agents see exactly
    what changed, can verify intent, and accept modifications without rewriting
    assertions

  - Purification of strings to avoid unstable dependencies
  - Conventions (e.g., factor out common code in helpers, ...)
  - Support for data science (e.g., checking complex dataframes)
  - Agent skills
  - Automatically generate a unit test from a cache
  - Utilities to trace system execution
  - Fuzzy matching
