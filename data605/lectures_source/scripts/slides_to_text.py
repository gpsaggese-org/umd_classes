...
def authenticate_and_get_creds() -> "Credentials":
    ...
    if GOOGLE_IMPORT_ERROR is not None:
        raise RuntimeError(
            f"Google API libraries are not available: {GOOGLE_IMPORT_ERROR}"
        )
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                logging.warning(
                    "Token refresh failed, falling back to new flow: %s", exc
                )
...
def download_images_in_order(
    creds: "Credentials",
    slide_data: List[dict],
    images_dir: str,
    lecture_number: float,
) -> dict:
    """
                ...
                if content_url:
                    try:
                        ...
                        if response.status == 200:
                            ...
                            lecture_safe = sanitize_lecture_number_for_filename(
                                lecture_number
                            )
                            ...
                            logging.warning(
                                f"Failed to download image from {content_url}, status: {response.status}"
                            )
                            image_map[object_id] = "image_download_failed"
                    except Exception as e:
                        logging.error(
                            f"Error downloading image from {content_url}: {e}"
                        )
                        image_map[object_id] = "image_download_failed"
    return image_map
    ...
    for element in text_elements:
        if "paragraphMarker" in element:
            ...
            if any(
                ...
                for part in current_paragraph["content_parts"]
            ):
        ...
        for c in range(n_cols):
            ...
            if not cells:
                table_data["cells"].append(
                    {"row": r, "col": c, "content": "", "formatted_content": []}
                )
                continue
            ...
            is_header = (r == 0) or cell.get("tableCellProperties", {}).get(
...
def extract_text_from_table(table: dict) -> str:
    ...
    for r in range(n_rows):
        ...
        for c in range(n_cols):
            ...
            cell = cells[0]
            paras = cell.get("text", {}).get("textElements")
            cell_text = extract_plain_text_from_paragraphs(
                extract_text_with_formatting(paras or [])
            )
...
def extract_slide_elements(slide: dict) -> List[dict]:
    ...
    for element in slide.get("pageElements", []):
        ...
        if "shape" in element:
            ...
            if placeholder:
                element_data.update(
                    {
                        "type": "placeholder",
                        "placeholder_type": placeholder.get(
                            "type", "UNSPECIFIED"
                        ),
            ...
            if text_elements:
                element_data.update(
                    {
                        "type": "text_shape",
                        "content": extract_plain_text_from_paragraphs(
                            extract_text_with_formatting(text_elements)
                        ),
                        "formatted_content": extract_text_with_formatting(
                            text_elements
                        ),
                    }
                )
            ...
            if (
                ...
                element_data.update(
                    {
                        "type": "image",
                        "image_properties": shape.get("shapeProperties", {})
                        .get("shapeFill", {})
                        .get("pictureFill", {}),
                    }
                )
        ...
        # Handle images
            ...
            element_data.update(
                {"type": "image", "image_properties": element.get("image", {})}
            )

        # Handle videos
        elif "video" in element:
            element_data.update(
                {"type": "video", "video_properties": element.get("video", {})}
            )
...
def extract_slide_text(slide: dict) -> str:
    ...
    for element in slide.get("pageElements", []):
        ...
        if placeholder and placeholder.get("type") == "TITLE":
            text_elements = shape.get("text", {}).get("textElements", [])
            title_text = extract_plain_text_from_paragraphs(
                extract_text_with_formatting(text_elements)
            )
            if title_text:
    ...
    for element in slide.get("pageElements", []):
        if "shape" in element:
            ...
            body_text = extract_plain_text_from_paragraphs(
                extract_text_with_formatting(text_elements)
            )
            if body_text:
...
def extract_notes_text(slide: dict) -> str:
    notes_page = slide.get("notesPage") or slide.get("slideProperties", {}).get(
        "notesPage", {}
    )
    note_texts: List[str] = []
...
def detect_content_type(text_content):
    ...
    if any(
        ...
        for keyword in ["instructor", "email", "references"]
    ):
        return "instructor_info"

    # Check for course header patterns
    if (
        any(keyword in text_content for keyword in ["MSML", "Course", ":"])
        and len(text_content.split()) < 8
    ):
                ...
                if element_type == "text_shape":
                    ...
                    for para in paragraphs:
                        ...
                        if para_content.strip():
                            ...
                            if para.get("bullet"):
                                indent_level = para["bullet"].get(
                                    "nestingLevel", 0
                                )
                                f.write(
                                    f"[BULLET:level={indent_level}] {para_content}\n"
                                )
                            else:
                    ...
                    f.write(
                        f"[TABLE {element.get('rows', 0)}x{element.get('columns', 0)}]\n"
                    )
                    for cell in element.get("cells", []):
                        ...
                        f.write(
                            f"[CELL:{row},{col}{'|HEADER' if is_header else ''}] {content}\n"
                        )
                    f.write("\n")
...
def process_one_link(
    ...
    try:
        ...
        logging.error(
            "Failed to fetch presentation %s: %s", presentation_id, exc
        )
        return None
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.urls:
        ...
        logging.info(
            "No URLs provided via command line, reading from links.txt..."
        )
        try:
    ...
    for i, url in enumerate(urls):
        lecture_num = (
            args.lecture_start + i
        )  # Calculate the actual lecture number
    ...
    logging.info(
        "Completed. %d/%d presentations processed successfully.",
        successes,
        len(urls),
    )
