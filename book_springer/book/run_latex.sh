...
LATEX_NAME=$SCRIPT_ABS_DIR/book.tex
echo "LATEX_NAME=$LATEX_NAME"

PDF_NAME=$(basename $(pwd)).pdf
echo "PDF_NAME=$PDF_NAME"

PDF_FILE_NAME=$SCRIPT_ABS_DIR/$PDF_NAME
...
if [[ $NUM_PASSES -ge 1 ]]; then
    (cd $SCRIPT_ABS_DIR && $DOCKERIZED_LATEX -i ${LATEX_NAME} -o $PDF_FILE_NAME)
fi;
...
if [[ $NUM_PASSES -ge 3 ]]; then
    if [[ -f $SCRIPT_ABS_DIR/references.bib ]]; then
