...
`pgmpy` is a Python library for probabilistic graphical models. In this tutorial
we build a causal Bayesian network, specify conditional probability tables,
perform inference, and simulate interventions using the do-operator.
...
# Define the sprinkler causal network.
# Nodes: Cloudy (C), Rain (R), Sprinkler (S), WetGrass (W)
...
### Step 2 — Specify Conditional Probability Distributions
```python
# Prior for Cloudy.
cpd_cloudy = TabularCPD(
    variable="Cloudy",
    variable_card=2,
    values=[[0.5], [0.5]],  # P(Cloudy=False), P(Cloudy=True)
)
# Rain given Cloudy.
cpd_rain = TabularCPD(
    variable="Rain",
    variable_card=2,
    values=[[0.8, 0.2],   # P(Rain=False | Cloudy=0,1)
            [0.2, 0.8]],  # P(Rain=True  | Cloudy=0,1)
    evidence=["Cloudy"],
    evidence_card=[2],
)
...
cpd_sprinkler = TabularCPD(
    variable="Sprinkler",
    variable_card=2,
    values=[[0.5, 0.9],   # P(Sprinkler=False | Cloudy=0,1)
            [0.5, 0.1]],  # P(Sprinkler=True  | Cloudy=0,1)
    evidence=["Cloudy"],
    evidence_card=[2],
)
# WetGrass given Rain and Sprinkler.
cpd_wet = TabularCPD(
    variable="WetGrass",
    variable_card=2,
    values=[[1.0, 0.1, 0.1, 0.01],   # P(Wet=False | R=0,S=0; R=0,S=1; R=1,S=0; R=1,S=1)
            [0.0, 0.9, 0.9, 0.99]],  # P(Wet=True  | ...)
    evidence=["Rain", "Sprinkler"],
    evidence_card=[2, 2],
)
model.add_cpds(cpd_cloudy, cpd_rain, cpd_sprinkler, cpd_wet)
print("Model valid:", model.check_model())
```

### Step 3 — Perform Associational Inference (Rung 1)
```python
# Standard Bayesian inference: P(Cloudy | WetGrass=True).
infer = VariableElimination(model)
result = infer.query(["Cloudy"], evidence={"WetGrass": 1})
print(result)
# Observing wet grass raises our belief that it is cloudy (via Rain or Sprinkler).
```

### Step 4 — Perform Interventional Inference (Rung 2)
```python
# Causal inference: P(WetGrass | do(Sprinkler=True)).
# The do-operator severs the Cloudy -> Sprinkler edge.
causal_infer = CausalInference(model)
result_do = causal_infer.query(
    variables=["WetGrass"],
    do={"Sprinkler": 1},
)
print(result_do)
# Forcing the sprinkler on directly affects WetGrass.
# Unlike observing Sprinkler=1, it provides no information about Cloudy.
```

### Step 5 — Check d-Separation
```python
# Is WetGrass d-separated from Cloudy given Rain?
# (Conditioning on Rain blocks the path Cloudy -> Rain -> WetGrass.)
is_dsep = model.local_independencies("Cloudy")
print(is_dsep)
# Use active_trail_nodes to see which variables are d-connected.
print(model.active_trail_nodes(["WetGrass"], observed=["Rain"]))
```

### Key Takeaways From the Pgmpy Tutorial
- `pgmpy` cleanly separates graph structure (DAG) from quantitative
  parameterization (CPDs)
- Standard `VariableElimination` answers Rung 1 questions (association)
- `CausalInference` implements the do-operator by mutilating the graph before
  inference
- D-Separation queries let you verify independence claims before running any
  estimation

### References
- Pgmpy documentation: https://pgmpy.org/
- Ankur Ankan and Abinash Panda, _Probabilistic Graphical Models with Python_
  (Packt, 2018)

## Tutorial: DAGitty (DAG Creation and Causal Effect Identification)
DAGitty is a browser-based and R-compatible tool for drawing causal DAGs and
automatically determining whether a causal effect is identifiable, which
variables to adjust for, and which variable sets satisfy the back-door or
front-door criteria. This tutorial demonstrates the workflow in R using the
`dagitty` package, with guidance for the browser interface.

### Prerequisites
```r
install.packages("dagitty")
install.packages("ggdag")   # For ggplot2-based DAG visualization.
library(dagitty)
library(ggdag)
```

### Step 1 — Define a Causal DAG
```r
# Specify the DAG as a string.
# Example: education (E) -> income (I), both confounded by family background (F)
# and individual motivation (M, unobserved).
dag <- dagitty("dag {
    FamilyBackground [pos=\"0,0\"]
    Education        [pos=\"1,1\", exposure]
    Income           [pos=\"2,1\", outcome]
    Motivation       [pos=\"1,0\", unobserved]
...
### Step 2 — Identify Adjustment Sets
```r
# Find all minimal adjustment sets that satisfy the back-door criterion.
adj_sets <- adjustmentSets(dag, exposure="Education", outcome="Income")
print(adj_sets)
# DAGitty reports which sets of observed variables, when conditioned on,
# allow identification of the causal effect Education -> Income.
# Here FamilyBackground alone suffices (Motivation is unobserved).
```

### Step 3 — Check d-Separation
```r
# Test whether Education and Income are d-separated given FamilyBackground.
dseparated(dag, "Education", "Income", c("FamilyBackground"))
# FALSE: Education and Income are still d-connected through Motivation (unobserved).
# This means FamilyBackground alone does not fully block confounding.
# The unobserved Motivation creates an unblockable back-door path.
```

### Step 4 — Identify Instrumental Variables
```r
# Find instrumental variables (if any) for the effect Education -> Income.
ivs <- instrumentalVariables(dag, exposure="Education", outcome="Income")
print(ivs)
```

### Step 5 — Visualize the DAG
```r
# Convert to tidy format and plot with ggdag.
tidy_dag <- tidy_dagitty(dag)
ggdag(tidy_dag) +
    theme_dag() +
    ggtitle("Education -> Income: Causal DAG")
# ggdag highlights the exposure, outcome, and adjustment sets.
```

### Using DAGitty in the Browser
- Navigate to https://www.dagitty.net/dags.html
- Draw nodes and edges interactively using the point-and-click interface
- Mark the exposure (treatment) and outcome variables
- DAGitty automatically:
  - Highlights all adjustment sets (back-door satisfying sets)
  - Marks colliders in red (do not condition on these)
  - Shows paths colored by whether they are causal or non-causal (back-door)
  - Reports whether the effect is identifiable

### Key Takeaways From the DAGitty Tutorial
- DAGitty enforces causal reasoning before any data analysis
- Adjustment set identification removes guesswork from confounder selection
- The tool surfaces collider structures that are easy to miss manually
- Combining DAGitty (for identification) with `pgmpy` or `dowhy` (for
  estimation) gives a principled end-to-end workflow

### References
- Johannes Textor et al., "Robust Causal Inference Using Directed Acyclic
  Graphs: The R Package dagitty," _International Journal of Epidemiology_ (2016)
- DAGitty web tool: https://www.dagitty.net/
- Ggdag package: https://ggdag.malco.io/
- Cinelli, C., Forney, A., and Pearl, J., "A Crash Course in Good and Bad
  Controls," _Sociological Methods and Research_ (2022)
